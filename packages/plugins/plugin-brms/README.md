...
Local-file Business Rules plugin for source ingestion, brms browsing, query, lint, and maintenance workflows.

## Scope

This package is the standalone home for Business Rules behavior. BRMS-specific routes,
...
- manifest-declared BRMS rule, sidebar entry, and settings rule
- trusted local folder declaration for `raw/`, `brms/`, `AGENTS.md`, `IDEA.md`, `brms/index.md`, and `brms/log.md`
- plugin database namespace migration for brms instances, sources, rules, operations, query sessions, and resource bindings
- managed `BRMS Maintainer` agent, managed `Business Rules` project, and paused managed routines for brms update processing, lint, and index refresh
...
- Paperclip-derived distillation (cursor windows, manual `distill-now`, backfill) always writes into the default brms space in Phase 1; non-default spaces remain on manual / raw-file ingest until per-space Paperclip ingestion profiles ship
- Paperclip-derived distillation maintains `brms/projects/<slug>/standup.md` as the executive current-state view for each represented project, alongside durable `brms/projects/<slug>/index.md` knowledge rules
- brms rule writes with plugin path validation, atomic local-folder writes, metadata/revision rows, backlink extraction, and optional stale-hash protection
- brms tools for search/read/write/propose patch/source/log/index/backlinks workflows
...
- The brms must not fetch `/api/assets/:id/content`, dereference work-product `url` fields, or store those capability-bearing links in source bundles/snapshots.

The accepted policy lives in [doc/plans/2026-05-06-brms-paperclip-asset-security-gate.md](../../../doc/plans/2026-05-06-brms-paperclip-asset-security-gate.md).
...
```bash
pnpm --filter @paperclipai/plugin-brms typecheck
pnpm --filter @paperclipai/plugin-brms test
pnpm --filter @paperclipai/plugin-brms build
```
...
- hidden plugin-operation issue creation for ingest/query/file-as-rule workflows
...
- atomic rule writes, metadata/revision rows, backlinks, and stale-hash refusal
- query session creation, run-id recording, stream event forwarding, and completion updates
- filing a streamed query answer back into the brms through a hidden operation
...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/Users/dotta/paperclip/.paperclip/worktrees/PAP-3179-design-a-brms-plugin/packages/plugins/plugin-brms","isLocalPath":true}'
```
...
Use the settings rule or `bootstrap-root` action to configure the folder and
...
default first-install skeleton is copied from the vanilla Business Rules layout, with
`CLAUDE.md` renamed to `AGENTS.md` and Paperclip project overviews, standups,
decisions, and history kept together under `brms/projects/<slug>/`.
...
For this plugin the BRMS Maintainer source bundle is `agents/brms-maintainer/AGENTS.md`.
