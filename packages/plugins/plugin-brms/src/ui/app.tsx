...
import { Suspense, useCallback, useEffect, useMemo, useRef, useState, type AnchorHTMLAttributes, type CSSProperties, type ReactElement, type ReactNode } from "react";
...
const PLUGIN_ID = "paperclipai.plugin-brms";
const BRMS_SIDEBAR_NAV_STATE_KEY = "paperclipBRMSSidebarTreePath";
const ROUTE_SIDEBAR_EXPANDED_STORAGE_PREFIX = `${PLUGIN_ID}:route-sidebar-expanded:v2`;
const BRMS_TOC_STICKY_TOP = 88;
const BRMS_SPACE_PREFETCH_LIMIT = 8;
...
type BRMSEventIngestionSource = "issues" | "comments" | "documents";
...
type BRMSSpaceWithFolderStatus = BRMSSpace & {
  relativeRoot: string;
  folder: FolderStatus;
};
...
type RulesData = {
  rules: BRMSRuleRow[];
  sources: BRMSSourceRow[];
};

type RuleContentData = {
  brmsId: string;
  path: string;
  contents: string;
  title: string | null;
...
type BRMSFrontmatterValue = string | string[];

type BRMSFrontmatterProperty = {
  key: string;
  value: BRMSFrontmatterValue;
};

type ParsedBRMSMarkdown = {
  body: string;
  frontmatter: BRMSFrontmatterProperty[];
};
...
function StatusIcon({ status }: { status: string }) {
  ...
  return (
    <span style={{
      ...
      animation: tone.pulse ? "pcBRMSPulse 1.6s infinite" : undefined,
    }} aria-hidden />
  );
}
  ...
  return (
    <div style={{ display: "grid", gap: 6, minWidth: 0 }}>
      <MarkdownEditor
        ...
        className="pc-brms-markdown-editor"
      />
      <style>{`
        .pc-brms-markdown-editor .mdxeditor-root-contenteditable {
...
function useRules(companyId: string | null, opts: { includeRaw?: boolean; spaceSlug?: string | null } = {}) {
  ...
  return usePluginData<RulesData>("rules", params);
}

function useSpaces(companyId: string | null) {
  const params = useMemo(() => companyId ? { companyId } : undefined, [companyId]);
  return usePluginData<BRMSSpacesData>("spaces", params);
}
...
function useSpaceFolderStatus(companyId: string | null, spaceSlug: string | null) {
  ...
  return usePluginData<BRMSSpaceWithFolderStatus>("space", params);
}
...
function useRuleContent(companyId: string | null, path: string | null, spaceSlug?: string | null) {
  const params = useMemo(() => {
  ...
  return usePluginData<RuleContentData>("rule-content", params);
}
...
type DistillationOverviewData = {
  ...
  ruleBindings: DistillationRuleBinding[];
...
type DistillationProvenanceData = {
  binding: DistillationRuleBinding | null;
  runs: DistillationRun[];
  snapshot: {
    id: string;
...
function useDistillationProvenance(companyId: string | null, rulePath: string | null) {
  const params = useMemo(() => {
    if (!companyId || !rulePath) return undefined;
    return { companyId, rulePath };
  }, [companyId, rulePath]);
  return usePluginData<DistillationProvenanceData>("distillation-rule-provenance", params);
}
...
function parseFrontmatterValue(rawValue: string, followingList: string[]): BRMSFrontmatterValue {
...
function parseBRMSFrontmatterBlock(block: string): BRMSFrontmatterProperty[] {
  const lines = block.replace(/\r\n/g, "\n").split("\n");
  const properties: BRMSFrontmatterProperty[] = [];
...
function parseBRMSMarkdown(contents: string): ParsedBRMSMarkdown {
  ...
  return {
    ...
    frontmatter: parseBRMSFrontmatterBlock(frontmatterBlock),
  };
}
...
function slugifyBRMSHeading(text: string): string {
...
function extractBRMSTocHeadings(markdownBody: string): BRMSTocHeading[] {
  const lines = markdownBody.replace(/\r\n/g, "\n").split("\n");
  const headings: BRMSTocHeading[] = [];
  ...
  for (const line of lines) {
    ...
    const baseId = slugifyBRMSHeading(text);
...
// ---------------------------------------------------------------------------
// Sidebar entry and settings rule.
// ---------------------------------------------------------------------------
...
export function SidebarLink({ context }: PluginSidebarProps) {
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 truncate">BRMS</span>
    </a>
  );
}

export function SettingsRule({ context }: PluginSettingsPageProps) {
...
// ---------------------------------------------------------------------------
// Main brms rule: section shell for Rules / Ask / Ingest / Lint / History /
// Settings. BRMS navigation uses path segments so rules can be deep-linked as
// `/:companyPrefix/brms/rule/path/to/file`. Legacy query-param links are still
...
const SECTIONS: ReadonlyArray<{
  ...
  description: string;
    ...
    { key: "browse", label: "BRMS", Icon: BookOpenIcon, description: "Open brms rules and raw sources from the sidebar." },
    { key: "query", label: "Ask", Icon: MessageSquareTextIcon, description: "Ask the BRMS Maintainer agent a cited question against the local brms." },
    ...
    { key: "lint", label: "Lint", Icon: ListChecksIcon, description: "Run structural checks for orphan rules, missing backlinks, and stale provenance." },
    { key: "history", label: "History", Icon: HistoryIcon, description: "Inspect recent Business Rules operation issues." },
...
function readBRMSRouteSegments(pathname: string): string[] {
  const segments = pathname.split("/").filter(Boolean);
  const brmsIndex = segments.findIndex((segment) => decodeRouteSegment(segment).toLowerCase() === "brms");
  if (brmsIndex === -1) return [];
  return segments.slice(brmsIndex + 1).map(decodeRouteSegment);
}

// Strip an optional `spaces/<slug>` prefix from the brms route segments.
// Returns the active space slug (defaults to "default") and the remaining segments
// after that prefix so existing section/rule parsing keeps working unchanged.
function readBRMSSpaceContext(pathname: string): { spaceSlug: string; rest: string[] } {
  const segments = readBRMSRouteSegments(pathname);
...
function readActiveSpaceSlugFromLocation(pathname: string): string {
  return readBRMSSpaceContext(pathname).spaceSlug;
}

function readSectionFromLocation(pathname: string, search: string): SectionKey {
  const [firstSegment] = readBRMSSpaceContext(pathname).rest;
  if (firstSegment === "rule") return "browse";
...
function readSettingsSectionFromLocation(pathname: string): SettingsSectionKey {
  const [firstSegment, secondSegment] = readBRMSSpaceContext(pathname).rest;
...
function readSettingsSpaceSlugFromLocation(pathname: string): string | null {
  const segs = readBRMSSpaceContext(pathname).rest;
...
function buildSpacePrefix(spaceSlug: string): string {
  return spaceSlug && spaceSlug !== DEFAULT_SPACE_SLUG
    ? `/brms/spaces/${encodeURIComponent(spaceSlug)}`
    : `/brms`;
}
...
function readSelectedTreePathFromSearch(search: string): string | null {
  const params = new URLSearchParams(search);
  const raw = params.get("rule")?.trim();
  return raw || null;
}

function readSelectedTreePathFromLocation(pathname: string, search: string): string | null {
  const [firstSegment, ...rest] = readBRMSSpaceContext(pathname).rest;
  if (firstSegment === "rule") {
    return treePathFromRouteSegments(rest);
  }
  return readSelectedTreePathFromSearch(search);
}

function buildRuleHref(treePath: string, spaceSlug: string = DEFAULT_SPACE_SLUG): string {
  const prefix = buildSpacePrefix(spaceSlug);
  const encodedPath = routeSegmentsFromTreePath(treePath).map((segment) => encodeURIComponent(segment)).join("/");
  return encodedPath ? `${prefix}/rule/${encodedPath}` : prefix;
}

function brmsSidebarNavigationState(treePath: string): Record<typeof BRMS_SIDEBAR_NAV_STATE_KEY, string> {
  return { [BRMS_SIDEBAR_NAV_STATE_KEY]: treePath };
}

function readSidebarSelectedPathFromNavigationState(state: unknown): string | null {
  if (!state || typeof state !== "object") return null;
  const value = (state as Record<string, unknown>)[BRMS_SIDEBAR_NAV_STATE_KEY];
...
const ROOT_BRMS_LINK_RULES = new Set(["BRMS.md", "AGENTS.md", "IDEA.md", "index.md", "log.md"]);

function splitBRMSLinkTarget(target: string): { path: string; fragment: string | null } | null {
...
function normalizeBRMSLinkRulePath(target: string): { path: string; fragment: string | null } | null {
  const parsed = splitBRMSLinkTarget(target);
  if (!parsed) return null;

  let path = withMarkdownExtension(parsed.path);
  if (!path.startsWith("brms/") && !path.startsWith("raw/") && !ROOT_BRMS_LINK_RULES.has(path)) {
    path = `brms/${path}`;
  }
...
function buildBRMSLinkHref(target: string, resolveHref: (to: string) => string): string | null {
  const normalized = normalizeBRMSLinkRulePath(target);
  if (!normalized) return null;

  const href = resolveHref(buildRuleHref(normalized.path));
...
function firstSelectableTreePath(data: RulesData | null | undefined): string | null {
  const firstRule = data?.rules.find((p) => p.path !== "brms/index.md" && p.path !== "index.md") ?? data?.rules[0] ?? null;
  if (firstRule) return firstRule.path;
...
function isEditableBRMSRulePath(path: string): boolean {
  return path === "BRMS.md"
    || path === "AGENTS.md"
    || path === "IDEA.md"
    || path === "index.md"
    || path === "log.md"
    || path.startsWith("brms/");
}

export function BRMSRule({ context }: PluginPageProps) {
  ...
  if (!context.companyId) {
    return <main style={{ ...shellStyle, height: isMobile ? "auto" : "100%", minHeight: isMobile ? "auto" : 600 }}>Choose a company to open the Business Rules.</main>;
  }
  ...
  return (
    <main
      ...
      onDrop={handleDrop}
    >
      <style>{`@keyframes pcBRMSPulse { 0%,100% { opacity:1 } 50% { opacity:0.45 } }`}</style>
      <section style={{ flex: 1, minHeight: isMobile ? "auto" : 0, overflow: isMobile ? "visible" : "hidden", display: "flex" }}>
        {overview.error ? (
          <div style={{ padding: 24, flex: 1 }}>
            <Callout tone="danger">Business Rules bridge error: {overview.error.message}</Callout>
          </div>
        ) : !overview.data ? (
          <div style={{ padding: 24, flex: 1, color: tokens.muted, fontSize: 13 }}>Loading brms…</div>
      ...
      {isDragActive ? <BRMSRuleDropOverlay onClose={resetDragState} /> : null}
      {isIngestModalOpen ? (
        <IngestFilesModal
          ...
          onIngested={() => {
            ...
            if (typeof window !== "undefined") {
              window.dispatchEvent(new CustomEvent("pc-brms-ingest-queued"));
            }
          }}
        />
      ) : null}
...
function BRMSRuleDropOverlay({ onClose }: { onClose: () => void }) {
  return (
    <div
      data-testid="brms-rule-drop-overlay"
      style={{
        position: "fixed",
        inset: 0,
        zIndex: 1000,
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ fontSize: 18, fontWeight: 650, marginBottom: 6 }}>Drop to ingest into Business Rules</div>
        <Tiny>Files will be staged for review before the brms maintainer queues ingest operations.</Tiny>
      </div>
    </div>
  );
}
...
function IngestFilesModal({
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="brms-ingest-modal-title"
      data-testid="brms-ingest-modal"
      onMouseDown={(event) => {
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 18px", borderBottom: `1px solid ${tokens.border}`, display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <h2 id="brms-ingest-modal-title" style={{ margin: 0, fontSize: 16, fontWeight: 650 }}>Ingest files into {targetSpace?.displayName ?? targetSpaceSlug}</h2>
            <Tiny style={{ marginTop: 4 }}>
              Review the staged files, switch the destination space if needed, then queue them as LLM
              BRMS ingest operations. This is manual file ingest - Paperclip-derived distillation always
        ...
        <div style={{ padding: 18, display: "grid", gap: 14 }}>
          ...
          <Callout>
            Confirming captures each file into <Mono>{targetSpaceSlug}/raw/</Mono>, attaches the original file to the ingest task, and initiates a task for the BRMS Maintainer to process.
          </Callout>
...
// ---------------------------------------------------------------------------
// BRMS route sidebar — replaces the company sidebar while the user is on a
// `/brms` route. Mirrors the shell of the host's CompanySettingsSidebar so
...
function CreateSpaceModal({
  ...
  onClose: () => void;
  onCreated: (space: BRMSSpace) => void;
}) {
  ...
  async function submit() {
    ...
    try {
      const result = await create({
        ...
        folderMode,
        accessScope,
      }) as { status: "created"; space: BRMSSpace };
      ...
      >
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${tokens.border}`, display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <Tiny style={{ marginTop: 4 }}>
              Spaces partition brms rules, sources, and manual ingest into separate slug-prefixed folders
              under the brms root. Paperclip distillation and event capture always write into the
        ...
        <div style={{ padding: 20, display: "grid", gap: 16 }}>
          ...
          <FormField label="Folder source" help="New managed folders create a slug-scoped subfolder under your brms root. Existing folders must already live under the same brms root.">
            <div style={{ display: "grid", gap: 8 }}>
              <FolderModeRow
                checked={folderMode === "managed_subfolder"}
                onSelect={() => setFolderMode("managed_subfolder")}
                label="New managed folder"
                help={`Creates spaces/${previewName.replace(/\s+/g, "-").toLowerCase()}/ under the configured brms root with the standard skeleton.`}
              />
              <FolderModeRow
                checked={folderMode === "existing_local_folder"}
                onSelect={() => setFolderMode("existing_local_folder")}
                label="Existing folder under brms root"
                help="Re-use a sub-folder you've already created inside the brms root. The folder path is stored in the space settings."
              />
              <FolderModeRow
                disabled
                checked={false}
...
function compareSpaces(a: BRMSSpace, b: BRMSSpace): number {
...
function activeBRMSSpaces(spaces: BRMSSpace[]): BRMSSpace[] {
...
function SpaceRuleContentWarmup({ companyId, path, spaceSlug }: { companyId: string | null; path: string; spaceSlug: string }) {
  useRuleContent(companyId, path, spaceSlug);
  return null;
}

function SpaceRulesWarmup({ companyId, spaceSlug }: { companyId: string | null; spaceSlug: string }) {
  const rules = useRules(companyId, { includeRaw: true, spaceSlug });
  const selectedTreePath = firstSelectableTreePath(rules.data);
  const selected = contentPathFromTreePath(selectedTreePath);
  return selected ? <SpaceRuleContentWarmup companyId={companyId} path={selected} spaceSlug={spaceSlug} /> : null;
}

export function BRMSRouteSidebar({ context }: PluginRouteSidebarProps) {
  const hostNavigation = useHostNavigation();
  ...
  const spaces = useMemo(() => {
    ...
    return activeBRMSSpaces(list).sort(compareSpaces);
  }, [spacesQuery.data]);
  const rules = useRules(context.companyId, { includeRaw: true, spaceSlug: activeSpaceSlug });
  const activeSpaceNodes = useMemo(
    () => buildBrowseTree(rules.data?.rules ?? [], rules.data?.sources ?? []),
    [rules.data],
  );
  const warmupSpaces = useMemo(() => {
    ...
    return (active ? [active, ...rest] : rest).slice(0, BRMS_SPACE_PREFETCH_LIMIT);
  ...
  return (
    <aside className="w-full h-full min-h-0 border-r border-border bg-background flex flex-col">
      {warmupSpaces.map((space) => (
        <SpaceRulesWarmup key={space.slug} companyId={context.companyId} spaceSlug={space.slug} />
      ))}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto border-t border-border px-3 py-3">
        <nav aria-label="BRMS primary" className="mb-3">
                  ...
                  <div style={{ paddingLeft: 18, marginTop: 2, marginBottom: 6 }}>
                    <FileTree
                      ...
                      onSelectFile={(path) => {
                        ...
                        hostNavigation.navigate(buildRuleHref(path, space.slug), { state: brmsSidebarNavigationState(path) });
                      }}
                      onToggleDir={handleToggleDir}
                      wrapLabels={false}
                      loading={rules.loading}
                      error={rules.error ? { message: rules.error.message } : null}
                      empty={{ title: "No rules yet", description: "Add content to populate this space." }}
                      ariaLabel={`BRMS rules in ${space.displayName}`}
                    />
                  </div>
                ) : null}
              </div>
...
function SpaceRowMenu({
  ...
  const handleArchive = async () => {
    ...
    if (typeof window !== "undefined" && !window.confirm(`Archive ${space.displayName}? Rules remain on disk; you can restore later through the plugin API or by un-archiving from the database.`)) {
      return;
    }
    setBusy(true);
    try {
...
function UnconfiguredFolder({ context, folder, refresh }: { context: { companyId: string | null }; folder?: FolderStatus; refresh: () => void }) {
  ...
  async function submit() {
    ...
    try {
      ...
      toast({ tone: "success", title: "BRMS root configured", body: written.length ? `Created ${written.length} bootstrap file(s).` : "Existing files preserved." });
      refresh();
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      setErrorMsg(message);
      toast({ tone: "error", title: "Could not configure brms root", body: message });
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div style={{ flex: 1, padding: isMobile ? 16 : 28, display: "grid", placeItems: "start", overflow: isMobile ? "visible" : "auto", minWidth: 0 }}>
      <Card style={{ maxWidth: 720, width: "100%" }}>
        <CardHeader title={configuredButUnhealthy ? "Repair brms root folder" : "Choose a brms root folder"} />
        <CardBody>
          <Tiny style={{ marginBottom: 12 }}>
            {configuredButUnhealthy
              ? "The configured brms root is not ready. Update the path or repair it to recreate required baseline files."
              : "Pick an absolute path on this machine. The plugin creates "}
            {!configuredButUnhealthy ? <><Mono>raw/</Mono>, <Mono>brms/</Mono>, <Mono>AGENTS.md</Mono>, <Mono>IDEA.md</Mono>, <Mono>brms/index.md</Mono>, and <Mono>brms/log.md</Mono> if they don't already exist.</> : null}
          </Tiny>
...
// ---------------------------------------------------------------------------
// Browse tab: selected rule detail. The route sidebar owns the file tree.
// ---------------------------------------------------------------------------

const TEMPLATE_PATHS = ["AGENTS.md", "IDEA.md"] as const;
const BASELINE_DIRECTORIES = ["raw", "brms", "brms/sources", "brms/projects", "brms/entities", "brms/concepts", "brms/synthesis"] as const;
const BASELINE_FILES = [...TEMPLATE_PATHS, "brms/index.md", "brms/log.md"] as const;
...
function ruleDisplayName(path: string, title: string | null): string {
  const trimmed = title?.trim();
  return trimmed ? trimmed : basename(path);
}

function treeDisplayName(path: string, title: string | null): string {
  return (BASELINE_FILES as readonly string[]).includes(path) ? basename(path) : ruleDisplayName(path, title);
}
...
function buildBrowseTree(
  ...
  if (sources.length > 0) {
    ...
    for (const source of sources) {
      ...
      const node: FileTreeNode = {
        name: ruleDisplayName(source.rawPath, source.title),
        path: source.rawPath,
        kind: "file",
        children: [],
      };
  ...
  for (const rule of rules) {
    ...
    const parentDir = dirname(rule.path);
    const file: FileTreeNode = {
      name: treeDisplayName(rule.path, rule.title),
      path: rule.path,
      kind: "file",
      children: [],
    };
  ...
  // Add the baseline files using their real brms-root paths so the browser
...
function BrowseTab({ context }: { context: { companyId: string | null } }) {
  ...
  const rules = useRules(context.companyId, { includeRaw: true, spaceSlug: activeSpaceSlug });
  const isMobile = useIsMobileLayout();
  const selectedTreePath = readSelectedTreePathFromLocation(pathname, search) ?? firstSelectableTreePath(rules.data);
  const selected = contentPathFromTreePath(selectedTreePath);

  return (
    <div style={{ flex: 1, minWidth: 0, overflow: isMobile ? "visible" : "auto" }}>
      {rules.loading && !selected ? (
        <div style={{ padding: isMobile ? 16 : 28, color: tokens.muted, fontSize: 13 }}>Loading rules…</div>
      ) : rules.error && !selected ? (
        <div style={{ padding: isMobile ? 16 : 28 }}><Callout tone="danger">Failed to load rules: {rules.error.message}</Callout></div>
      ) : (
        <RuleDetail context={context} path={selected} spaceSlug={activeSpaceSlug} />
      )}
    </div>
  );
}

function RuleDetail({ context, path, spaceSlug }: { context: { companyId: string | null }; path: string | null; spaceSlug?: string }) {
  const content = useRuleContent(context.companyId, path, spaceSlug ?? null);
  const writeRule = usePluginAction("write-rule");
  const toast = usePluginToast();
  const hostNavigation = useHostNavigation();
  const isMobile = useIsMobileLayout();
  ...
  const parsedMarkdown = useMemo(() => parseBRMSMarkdown(content.data?.contents ?? ""), [content.data?.contents]);
  const tocHeadings = useMemo(() => extractBRMSTocHeadings(parsedMarkdown.body), [parsedMarkdown.body]);
  ...
  const editable = path ? isEditableBRMSRulePath(path) : false;
  const resolveBRMSLinkHref = useCallback(
    (target: string) => buildBRMSLinkHref(target, hostNavigation.resolveHref),
  ...
  const saveRuleContents = useCallback(async (nextContents: string) => {
    if (!context.companyId || !content.data || !editable || !path) return;
    const result = await writeRule({
      companyId: context.companyId,
      brmsId: content.data.brmsId,
      spaceSlug: spaceSlug ?? null,
      path,
      contents: nextContents,
      expectedHash: savedHash ?? content.data.hash,
      summary: `Edited ${path} from the Business Rules rule`,
    }) as { hash?: string };
    if (typeof result.hash === "string") setSavedHash(result.hash);
  }, [context.companyId, content.data, editable, path, savedHash, writeRule, spaceSlug]);

  if (!path) return <div style={{ padding: isMobile ? 16 : 28, color: tokens.muted, fontSize: 13 }}>Pick a rule from the tree.</div>;
  ...
  if (content.error && path.startsWith("raw/")) {
    return (
      <div style={{ padding: isMobile ? 16 : 28, display: "grid", gap: 12 }}>
        <Callout tone="warn">
          The captured source <Mono>{path}</Mono> is indexed but no longer exists in the configured brms folder. Refresh the brms or re-ingest the source to restore it.
        </Callout>
  ...
  const isDistilledProjectRule = path.startsWith("brms/projects/");
  ...
  return (
    <article style={{ padding: isMobile ? "16px" : "24px 28px", display: "grid", gap: isMobile ? 12 : 14, minWidth: 0 }}>
      <header style={{ display: "grid", gap: 8, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, flexWrap: "wrap", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            {editable && !editing ? (
              <Button size="sm" onClick={() => { setSavedHash(hash); setEditing(true); }}>Edit rule</Button>
            ) : editable ? (
      ...
      {isDistilledProjectRule && path ? (
        <FreshnessChip companyId={context.companyId} rulePath={path} companyPrefix={(context as { companyPrefix?: string | null }).companyPrefix ?? null} />
      ) : null}
      {editing ? (
        <>
          <AutosaveMarkdownEditor
            ...
            onSave={async (nextContents) => {
              await saveRuleContents(nextContents);
        ...
        <div
          data-testid="brms-rule-content-layout"
          style={{
            display: "grid",
...
function OnThisRulePane({
  ...
  const contentId = "brms-on-this-rule";
  ...
  useEffect(() => {
    ...
    const updateFrame = () => {
      ...
      animationFrame = requestAnimationFrame(() => {
        ...
        const top = Math.max(BRMS_TOC_STICKY_TOP, rect.top);
  ...
  const paneStyle: CSSProperties = mobile ? {} : fixedFrame ? {
    ...
    position: "sticky",
    top: BRMS_TOC_STICKY_TOP,
  };

  return (
    <aside
      ref={shellRef}
      aria-label="On this rule"
      style={{
        order: mobile ? -1 : 0,
        minWidth: 0,
    ...
    >
      <div style={{
        ...
        <button
          type="button"
          aria-label={open ? "Collapse on this rule" : "Expand on this rule"}
          aria-expanded={open}
        ...
        >
          {open ? <span>On this rule</span> : null}
...
function FreshnessChip({ companyId, rulePath, companyPrefix }: { companyId: string | null; rulePath: string; companyPrefix: string | null }) {
  const provenance = useDistillationProvenance(companyId, rulePath);
...
function ProvenanceDrawer({ companyId, rulePath, onClose }: { companyId: string | null; rulePath: string; onClose: () => void }) {
  const isMobile = useIsMobileLayout();
  const provenance = useDistillationProvenance(companyId, rulePath);
    ...
    >
      ...
      >
        ...
        <div style={{ overflow: "auto", flex: 1, padding: 18, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {!binding && !provenance.loading ? <Callout>This rule is not currently bound to a distillation cursor. It may be hand-authored or pre-distillation.</Callout> : null}
          {binding ? (
            <Card>
              <CardBody padding={14}>
                <PropRow label="Rule path" value={<Mono>{binding.rulePath}</Mono>} />
          ...
          {runs.length > 0 ? (
            <Card>
              <CardHeader title="Operations affecting this rule" />
              <CardBody padding={0}>
...
function FrontmatterProperties({ properties }: { properties: BRMSFrontmatterProperty[] }) {
...
function FrontmatterValue({ value }: { value: BRMSFrontmatterValue }) {
...
function IngestTab({ context, refreshOverview }: { context: { companyId: string | null }; refreshOverview: () => void }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("pc-brms-ingest-queued", refresh);
    return () => window.removeEventListener("pc-brms-ingest-queued", refresh);
  }, [refreshOverview]);
    ...
    <div style={{ flex: 1, minHeight: isMobile ? "auto" : 0, minWidth: 0, overflow: isMobile ? "visible" : "auto" }}>
      <div style={{
        ...
        <Tiny style={{ marginBottom: 18 }}>
          Each capture queues an ingest operation scoped to <Mono>{activeSpaceSlug}</Mono>. Files land in that space's <Mono>raw/</Mono> folder and the BRMS Maintainer proposes a patch.
        </Tiny>
        <div style={{ display: "grid", gap: 14, marginBottom: 18 }}>
          ...
          />
          <div>
            <label style={{ fontSize: 11, color: tokens.muted, display: "block", marginBottom: 6 }}>Drop files anywhere on this rule</label>
            <div style={{
          ...
          <div data-testid="brms-ingest-manual-separator" aria-hidden="true" style={{ display: "flex", alignItems: "center", gap: 12, color: tokens.muted, fontSize: 11, fontWeight: 650, textTransform: "uppercase", letterSpacing: "0.04em" }}>
          ...
          <div>
            ...
            <TextInput value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Karpathy Business Rules gist" />
          </div>
          <div>
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      <div style={{ marginTop: 6, fontSize: 11, color: tokens.muted, lineHeight: 1.4 }}>
        Defaults to the space you opened. Switching now re-routes the rule so deep links carry the destination.
      </div>
      {isOpen ? (
        <div
          role="listbox"
...
function OperationCard({ op }: { op: BRMSOperationRow }) {
  return (
    <Card>
      <div style={{ padding: "12px 14px" }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", minWidth: 0 }}>
          <StatusIcon status={op.status} />
          <strong style={{ flex: "1 1 180px", minWidth: 0, fontSize: 13, overflowWrap: "anywhere" }}>{op.hiddenIssueTitle ?? `Business Rules ${op.operationType}`}</strong>
...
function QueryTab({ context, overview }: { context: { companyId: string | null }; overview: OverviewData }) {
  ...
  const fileAsRule = usePluginAction("file-as-rule");
  const toast = usePluginToast();
  const { pathname } = useHostLocation();
  const activeSpaceSlug = useMemo(() => readActiveSpaceSlugFromLocation(pathname), [pathname]);
  const isMobile = useIsMobileLayout();
  ...
  const [busy, setBusy] = useState(false);
  const [filePath, setFilePath] = useState("brms/concepts/new-rule.md");
  ...
  const stream = usePluginStream<QueryStreamEvent>(activeEntry?.channel ?? "brms:idle", {
    ...
    try {
      ...
      setThread((prev) => prev.map((entry) =>
        entry.id === entryId ? {
          ...
          channel: result.channel ?? `brms:query:${result.operationId}`,
          status: "running",
        } : entry,
      ));
      setPrompt("");
  ...
  async function fileAnswer(entry?: QueryThreadEntry) {
    ...
    try {
      ...
      toast({ tone: "success", title: "Answer filed", body: `Wrote ${filePath.trim()} and recorded a file-as-rule task.` });
      setFileBody("");
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
  ...
  return (
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", flex: 1, minHeight: isMobile ? "auto" : 0, minWidth: 0 }}>
      <div style={{ flex: 1, padding: isMobile ? "16px" : "24px 28px", overflow: isMobile ? "visible" : "auto", minWidth: 0 }}>
        {thread.length === 0 ? (
          <Callout>
            Ask the brms anything. Each question initiates a task assigned to the BRMS Maintainer. The answer streams below; you can promote useful answers into a brms rule.
          </Callout>
        ) : null}
              ...
              {entry.answer.trim() && entry.status === "done" ? (
                <div style={{ marginTop: 10, border: `1px dashed ${tokens.border}`, borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <strong style={{ fontSize: 13 }}>📑 File this answer as a brms rule?</strong>
        ...
        <div style={{ borderTop: `1px solid ${tokens.border}`, paddingTop: 14, marginTop: 22 }}>
          <TextArea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Ask the brms…" rows={3} />
          <div style={{ display: "flex", gap: 6, marginTop: 8, alignItems: "center", flexWrap: "wrap" }}>
            <Button variant="primary" size="sm" onClick={send} disabled={!prompt.trim()} loading={busy}>Send (⌘↵)</Button>
            <Badge>Cite: brms + raw</Badge>
      ...
      <aside style={{
        ...
        <PropRow label="BRMS" value={overview.brmsId} />
...
function SettingsLintPanel({ context }: { context: { companyId: string | null } }) {
  ...
  if (overview.error) {
    return <SettingsPanel title="Lint" badge={<HiddenOpBadge />} description="Run structural checks for orphan rules, missing backlinks, and stale provenance.">
      <Callout tone="danger">Business Rules bridge error: {overview.error.message}</Callout>
    </SettingsPanel>;
  }
  ...
  return (
    <SettingsPanel
      title="Lint"
      badge={<HiddenOpBadge />}
      description="Run structural checks for orphan rules, missing backlinks, and stale provenance."
    >
  ...
  async function runLint() {
    ...
    try {
      await create({
        ...
        spaceSlug: activeSpaceSlug,
        operationType: "lint",
        title: `Run Business Rules lint · ${activeSpaceSlug}`,
        prompt: overview.prompts.lint,
      });
      toast({ tone: "success", title: "Lint queued", body: "Lint runs as a BRMS Maintainer task. Findings will appear here once the run completes." });
      operations.refresh();
      refreshOverview();
    } catch (err) {
  ...
  return (
    <div style={{ display: "grid", gap: isMobile ? 14 : 18, minWidth: 0 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12 }}>
        ...
        <StatCard label="Index drift" value={String(counts.index)} hint="brms/index.md / brms/log.md" />
      </div>
      <Card style={unfilledSurfaceStyle}>
        <CardHeader title="Findings" badges={<HiddenOpBadge />} right={<Tiny>Lint runs as a BRMS Maintainer task. Critical findings can optionally open visible follow-up issues — toggle in Settings → Lint policy.</Tiny>} />
        <CardBody padding={0}>
          {findings.length === 0 ? (
      ...
      <Card style={unfilledSurfaceStyle}>
        ...
        <CardBody padding={0}>
          {recent.length === 0 ? <div style={{ padding: 12, color: tokens.muted, fontSize: 13 }}>No lint runs yet.</div> : recent.map((op) => (
            <Row
              key={op.id}
              primary={<><Mono>op-{op.id.slice(0, 6)}</Mono> {op.hiddenIssueTitle ?? "BRMS lint"}</>}
...
// ---------------------------------------------------------------------------
// History tab: native Paperclip issue table for recent Business Rules operation
...
function HistoryTab({ context, overview }: { context: { companyId: string | null; companyPrefix?: string | null }; overview: OverviewData }) {
  ...
  if (!context.companyId) {
    return <div style={{ padding: isMobile ? 16 : 24, flex: 1 }}><Callout>Choose a company to view Business Rules history.</Callout></div>;
  }

  if (!projectId) {
    return (
      <div style={{ padding: isMobile ? 16 : 24, flex: 1 }}>
        <Callout tone="warn">The Business Rules operations project is not resolved yet. Reconcile the managed project in Settings, then history will show its issues here.</Callout>
      </div>
    );
  }
  ...
  return (
    <div style={{ flex: 1, minHeight: isMobile ? "auto" : 0, overflow: isMobile ? "visible" : "auto", padding: isMobile ? 12 : 0, minWidth: 0 }}>
      <PluginIssuesList
        ...
        viewStateKey="paperclip:brms-history-issues-view"
...
// ---------------------------------------------------------------------------
// Settings (tab) and the standalone host SettingsRule share this body.
// ---------------------------------------------------------------------------
...
const ROUTINE_FALLBACKS: Record<string, { title: string; cron: string }> = {
  "cursor-window-processing": { title: "Process Business Rules updates", cron: "0 */6 * * *" },
  "nightly-brms-lint": { title: "Run Business Rules lint", cron: "0 3 * * *" },
  "index-refresh": { title: "Refresh Business Rules index", cron: "0 * * * *" },
};

const MANAGED_SKILL_LABELS: Record<string, string> = {
  "brms-maintainer": "Business Rules Maintainer",
  "brms-ingest": "BRMS Ingest",
  "brms-query": "BRMS Query",
  "brms-lint": "BRMS Lint",
...
function buildAgentHealthItems(managedAgent: ManagedAgent): RoutineHealthItem[] {
  const agentName = managedAgent.details?.name ?? "BRMS Maintainer";
  return [{
    label: agentName,
    ...
    detail: managedAgent.source === "managed"
      ? managedAgent.defaultDrift?.changedFiles.length
        ? `The BRMS Maintainer instructions differ from the plugin default: ${managedAgent.defaultDrift.changedFiles.join(", ")}.`
        : "The plugin-managed BRMS Maintainer exists with current default instructions."
      : "The settings rule is using a selected maintainer instead of the plugin-managed BRMS Maintainer.",
  }];
}

function buildProjectHealthItems(managedProject: ManagedProject): RoutineHealthItem[] {
  const projectName = managedProject.details?.name ?? "Business Rules";
  return [{
    label: projectName,
    ok: managedProjectIsReady(managedProject),
    detail: managedProject.source === "managed"
      ? "The plugin-managed Business Rules project exists."
      : "The settings rule is using a selected project instead of the plugin-managed Business Rules project.",
  }];
}
...
function buildRoutineHealthItems(
  ...
  for (const [key, fallback] of Object.entries(ROUTINE_FALLBACKS)) {
    ...
    let detail = `${fallback.title} is installed with the BRMS Maintainer and Business Rules project.`;
    if (missing) {
      ...
      detail = `${fallback.title} is not assigned to the BRMS Maintainer.`;
    } else if (wrongProject || missingProject) {
      detail = `${fallback.title} is not attached to the Business Rules project.`;
    }
...
function RoutineHealthChecklist({ items }: { items: RoutineHealthItem[] }) {
  return (
    <ManagedResourceHealthChecklist
      items={items}
      ariaLabel="BRMS routines health checklist"
...
function SkillHealthChecklist({ items }: { items: RoutineHealthItem[] }) {
  return (
    <ManagedResourceHealthChecklist
      items={items}
      ariaLabel="BRMS skills health checklist"
...
const SETTINGS_SECTIONS: ReadonlyArray<{
  ...
  description: string;
    ...
    { key: "routines", label: "Managed Routines", description: "Scheduled brms maintenance." },
    { key: "lint", label: "Lint", description: "Run checks and review brms health findings." },
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="brms-path-help-title"
        onClick={(event) => event.stopPropagation()}
        style={{
          width: "min(460px, 100%)",
          border: `1px solid ${tokens.border}`,
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start" }}>
          <div style={{ display: "grid", gap: 4 }}>
            <h3 id="brms-path-help-title" style={{ margin: 0, fontSize: 15, fontWeight: 650 }}>Get a full folder path</h3>
            <Tiny>Paste an absolute path such as <Mono>/Users/you/company-brms</Mono>.</Tiny>
          </div>
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderBottom: `1px solid ${tokens.border}` }}>
        ...
        <div style={{ display: "grid", gap: 2, minWidth: 0 }}>
          <span style={{ fontSize: 13, fontWeight: 650 }}>Local brms folder</span>
      ...
      <div style={{ display: "flex", gap: 8, padding: 12, alignItems: "center", flexWrap: "wrap" }}>
        <TextInput
          ...
          placeholder="/absolute/path/to/brms-root"
...
function folderHealthItems(folder: FolderStatus): FolderHealthItem[] {
  return [
    ...
    { label: "BRMS folders", ok: folder.missingDirectories.length === 0 },
  ];
}
...
function FolderHealthChecklist({ folder }: { folder: FolderStatus }) {
  ...
  return (
    <div style={{ display: "grid", gap: 8 }} aria-label="BRMS root health checklist">
  ...
  async function runDistillNow() {
    ...
    try {
      ...
      toast({
        ...
        body: "BRMS Maintainer will scan changed projects in the company and write into the default brms space.",
      });
      overview.refresh();
    } catch (err) {
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 12 }}>
              ...
              <div>
                ...
                <Tiny style={{ marginTop: 6, fontSize: 13, color: tokens.fg, lineHeight: 1.55, maxWidth: 540 }}>
                  When enabled, the BRMS Maintainer reads Paperclip issues, comments, and documents for this
                  company and keeps <Mono>brms/projects/&lt;slug&gt;/standup.md</Mono> plus <Mono>brms/projects/&lt;slug&gt;/index.md</Mono> rules in the
                  <strong> default brms space</strong>. Rules stay marked stale until a cursor window succeeds -
                  they never imply live state.
                </Tiny>
                <Tiny style={{ marginTop: 6, fontSize: 13, color: tokens.fg, lineHeight: 1.55, maxWidth: 540 }}>
                  Other spaces do not receive Paperclip-derived rules yet. They stay on manual and raw-file
          ...
          <Card>
            ...
            <CardBody padding={14}>
              <ul style={{ margin: 0, paddingLeft: 18, fontSize: 12.5, color: tokens.muted, lineHeight: 1.5 }}>
                <li>Project overviews at <Mono>brms/projects/&lt;slug&gt;/index.md</Mono></li>
                <li>Executive standups at <Mono>brms/projects/&lt;slug&gt;/standup.md</Mono></li>
                <li>Decisions and history under <Mono>brms/projects/&lt;slug&gt;/</Mono></li>
    ...
    <div style={{ display: "grid", gap: 16, minWidth: 0 }}>
      ...
      <Callout>
        ...
        <Tiny style={{ marginTop: 6 }}>
          Distillation runs on the assigned BRMS Maintainer agent and writes only into the default
      ...
      <Card>
        ...
        <CardBody padding={14}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
            <SettingField label="Max source characters per project" hint="Source bundles above this size are clipped and the rule warns 'source clipped'.">
            ...
            <SettingField label="Stale window threshold" hint="After this, project rules render a 'Stale' badge until a successful run advances the cursor.">
...
function SettingsBody({ context, initialSection = "root" }: { context: { companyId: string | null; companyPrefix?: string | null }; initialSection?: SettingsSectionKey }) {
  ...
  if (!context.companyId) return <Callout>Choose a company to view Business Rules settings.</Callout>;
  ...
  const maintainerFallbackAgent: MaintainerAgentOption | null = data.managedAgent.agentId
    ? {
      ...
      name: data.managedAgent.details?.name ?? "BRMS Maintainer",
  ...
  const configurationErrors = [
    ...(!data.folder.healthy ? ["BRMS root folder"] : []),
  ...
  const managedRoutineItems: ManagedRoutinesListItemWithDrift[] = managedRoutines.map((routine) => {
    ...
    return {
      ...
      managedByPluginDisplayName: routine.routine?.managedByPlugin?.pluginDisplayName ?? "Business Rules",
  ...
  async function fixAllConfigurationErrors() {
    ...
    const confirmed = typeof window === "undefined" || window.confirm(
      "Fix all detected Business Rules configuration errors? This may recreate missing brms baseline files and restore plugin-managed agents, projects, routines, and skills to their current defaults.",
    );
    if (!confirmed) return;
    ...
    try {
      if (!data.folder.healthy) {
        ...
        if (!path && !data.folder.configured) {
          throw new Error("Choose a brms root folder path before fixing all configuration errors.");
        }
  ...
  async function resetManagedRoutineToDefaults(routine: ManagedRoutinesListItem) {
    ...
    const confirmed = typeof window === "undefined" || window.confirm(
      `Update "${routine.title}" to the current Business Rules plugin defaults? This replaces ${fieldList}. Cancel to keep the current custom routine text.`,
    );
    if (!confirmed) return;
  ...
  async function resetManagedAgentToDefaults() {
    ...
    const confirmed = typeof window === "undefined" || window.confirm(
      `Update the BRMS Maintainer to the current Business Rules plugin defaults? This replaces ${fileList}. Cancel to keep the current custom instructions.`,
    );
    if (!confirmed) return;
  ...
  return (
    <div style={{
      ...
      <aside style={{
        ...
        <nav aria-label="Business Rules settings sections" style={{
          display: "flex",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {activeSettingsSection === "root" ? (
            ...
            <SetupSection title="Managed Agents" separated>
              <div style={{ display: "grid", gap: 14, maxWidth: isMobile ? "none" : 620, minWidth: 0 }}>
                <ManagedResourceHealthChecklist
                  ...
                  ariaLabel="BRMS agents health checklist"
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  {maintainerPendingApproval ? (
                    <Callout tone="warn">
                      The BRMS Maintainer is pending approval. Approve the agent before relying on brms ingest, query, lint, or scheduled maintenance tasks.
                    </Callout>
                  ) : null}
                  {showMaintainerWarning ? (
                    <Callout tone="warn">
                      This is not the Paperclip-provided BRMS Maintainer. Plugin operations and routines may miss the recommended brms role, tools, and default instructions.
                    </Callout>
                  ) : null}
                  {agentDefaultDrift?.changedFiles.length ? (
                    <Callout tone="warn">
                      BRMS Maintainer instruction defaults changed: {agentDefaultDrift.changedFiles.join(", ")}. Reset only if you want to replace current custom instructions with the plugin template.
                    </Callout>
                  ) : null}
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <Button
            ...
            <SetupSection title="Managed Skills" separated>
              <div style={{ display: "grid", gap: 12, maxWidth: isMobile ? "none" : 620, minWidth: 0 }}>
                ...
                {skillHealthWarnings.length > 0 ? (
                  ...
                  <Callout>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                      <span>Business Rules skills are installed in the company skill library.</span>
            ...
            <SetupSection title="Managed Projects" separated>
              <div style={{ display: "grid", gap: 10, maxWidth: isMobile ? "none" : 620, minWidth: 0 }}>
                <ManagedResourceHealthChecklist
                  ...
                  ariaLabel="BRMS projects health checklist"
            ...
            <SetupSection title="Managed Routines" separated>
              <div style={{ display: "grid", gap: 12, maxWidth: isMobile ? "none" : 620, minWidth: 0 }}>
                ...
                {routineHealthWarnings.length > 0 ? (
                  ...
                  <Tiny>Managed routines are installed with the BRMS Maintainer and Business Rules project.</Tiny>
                )}
              </div>
            </SetupSection>
          </section>
          ...
          <SettingsPanel
            ...
            description="Read Paperclip issues, comments, and documents for this company and write project rules into the default brms space. Assets/attachments and work products stay metadata-only in Phase 5 and are excluded from source-text extraction. Other spaces cannot be selected as a destination yet - that lands with per-space Paperclip ingestion profiles."
          >
          ...
          <SettingsPanel title="Managed Routines" description={activeSettingsConfig.description}>
            <div style={{ display: "grid", gap: 12 }}>
              {routineDefaultDriftItems.length > 0 ? (
                <Callout tone="warn">
                  <div style={{ display: "grid", gap: 6 }}>
                    ...
                    <span>
                      Review rows marked with changed defaults. Reset a row to update it to the current Business Rules instructions, or leave it unchanged to keep custom routine text.
                    </span>
                  </div>
                </Callout>
              ) : null}
              {routineHealthWarnings.length > 0 ? (
                ...
                <Callout>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    <span>Managed routines are installed with the BRMS Maintainer and Business Rules project.</span>
              ...
              <PluginManagedRoutinesList
                ...
                pluginDisplayName="Business Rules"
          ...
          >
            <Tiny style={{ marginBottom: 10 }}>
              Company-scoped Paperclip events can advance default-space cursors. Enable only the first-party text sources this brms should observe for default-space distillation.
            </Tiny>
            <div style={{ display: "grid", gap: 10 }}>
              ...
              <Callout tone="warn">
                Event ingestion records selected Paperclip issue, comment, and document activity for the default brms space. Assets/attachments and work products are excluded here: Phase 5 allows metadata-only references later, not blob reads or linked-content fetches. It never reads across companies or creates non-default space cursors.
              </Callout>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
...
// Reachable via /brms/settings/spaces or /brms/settings/spaces/<slug>.
// ---------------------------------------------------------------------------
...
function SpaceEditCard({
  ...
  const folderModeLabel = space.folderMode === "managed_subfolder"
    ? "New managed folder (under brms root)"
    : space.folderMode === "existing_local_folder"
      ? "Existing folder under brms root"
      : space.folderMode;
  ...
  async function archive() {
    ...
    if (typeof window !== "undefined" && !window.confirm(`Archive ${space.displayName}? Rules stay on disk; you can restore later through the plugin API or by un-archiving from the database.`)) {
      return;
    }
    setArchiveBusy(true);
    try {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <Card>
        ...
        <CardBody>
          <Tiny>
            Stored under <Mono>{relativeRoot || (isDefault ? "(brms root)" : `spaces/${space.slug}/`)}</Mono> within the configured brms root.
          </Tiny>
        </CardBody>
      </Card>
      ...
      <Card>
        ...
        <CardBody>
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <FormField label="Slug" help="Slug is locked once a space has indexed rules. Contact platform team to migrate.">
            ...
            <Tiny>
              {isDefault
                ...
                : isOnlySpace
                  ...
                  : "Archiving hides the space from the sidebar and pauses scheduled lint/index. Rules remain on disk."}
            </Tiny>
            <div>
              <Button
                variant="destructive"
...
function paperclipIngestionStateBadge(data: PaperclipIngestionProfileData | null): { tone: Tone; label: string } {
  ...
  return { tone: "default", label: data.historicalRuleCount > 0 ? `Off · ${data.historicalRuleCount} historical rules` : "Off" };
}

function PaperclipIngestionSpaceCard({ companyId, space, refresh }: { companyId: string | null; space: BRMSSpace; refresh: () => void }) {
  ...
  function setSourceKind(key: BRMSEventIngestionSource, value: boolean) {
    ...
    <Card>
      ...
      <CardBody>
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {locked ? (
            <Callout tone="warn">
              Locked — host permissions pending. Paperclip ingestion stays disabled on team and personal spaces until Business Rules enforces read/write permissions for non-shared spaces.
            </Callout>
          ) : null}
          {data && data.historicalRuleCount > 0 && data.effectiveState === "disabled" ? (
            <Callout>
              Off · {data.historicalRuleCount} historical Paperclip rule{data.historicalRuleCount === 1 ? "" : "s"} still in this space. Disabling stops new observations but does not delete prior brms rules.
            </Callout>
          ) : null}
            ...
            <>
              <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 13 }}>
                ...
                <span>
                  ...
                  <Tiny style={{ display: "block" }}>Future Paperclip issue, comment, and document events can advance cursors in {space.displayName}. Existing rules are preserved when this is turned off.</Tiny>
                </span>
              </label>
              <div style={{ display: "grid", gap: 8 }}>
