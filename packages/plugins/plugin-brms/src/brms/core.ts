...
import {
  ...
  LINT_PROMPT,
  QUERY_PROMPT,
  REQUIRED_BRMS_DIRECTORIES,
  REQUIRED_BRMS_FILES,
} from "../templates.js";

export const DEFAULT_BRMS_ID = "default";
...
const EVENT_INGESTION_STATE_NAMESPACE = "brms";
const EVENT_INGESTION_STATE_KEY = "event-ingestion";
const EVENT_INGESTION_DEDUP_NAMESPACE = "brms-event-ingestion";
...
const PROTECTED_BRMS_CONTROL_FILES = new Set(["AGENTS.md", "IDEA.md"]);
...
export type BRMSEventIngestionSource = "issues" | "comments" | "documents";
...
export type BRMSEventIngestionSettings = {
  enabled: boolean;
  sources: Record<BRMSEventIngestionSource, boolean>;
  brmsId: string;
  maxCharacters: number;
};

export type BRMSEventIngestionSettingsUpdate = Omit<Partial<BRMSEventIngestionSettings>, "sources"> & {
  sources?: Partial<Record<BRMSEventIngestionSource, boolean>>;
};
...
export type PaperclipIngestionProfileV1 = {
  ...
  sourceKinds: Record<BRMSEventIngestionSource, boolean> & {
...
export type PaperclipIngestionProfileRead = {
  brmsId: string;
  space: Pick<BRMSSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
...
type PaperclipIngestionPolicyDecision =
  | { allowed: true; space: BRMSSpace }
  | { allowed: false; space: BRMSSpace; reason: "restricted_space" | "archived_space" | "profile_disabled" | "profile_empty"; message: string };
...
export type EnableActiveProjectDistillationResult = {
  ...
  eventIngestion: BRMSEventIngestionSettings;
};

export const DEFAULT_EVENT_INGESTION_SETTINGS: BRMSEventIngestionSettings = {
  enabled: false,
  sources: {
    issues: false,
    comments: false,
  ...
  brmsId: DEFAULT_BRMS_ID,
  maxCharacters: 12000,
};

export type BRMSOverview = {
  status: "ok";
  checkedAt: string;
  brmsId: string;
  defaultSpace: BRMSSpace;
  folder: Awaited<ReturnType<PluginContext["localFolders"]["status"]>>;
  managedAgent: BRMSAgentResource;
  managedProject: BRMSProjectResource;
...
export type BRMSProjectResource = {
  status: "missing" | "resolved" | "created" | "relinked" | "reset";
  source: "managed" | "selected";
...
export type BRMSResourceOption = {
  id: string;
  name: string;
  status?: string | null;
...
export type BRMSSpaceWithFolderStatus = BRMSSpace & {
  relativeRoot: string;
  folder: Awaited<ReturnType<PluginContext["localFolders"]["status"]>>;
};
...
type OperationInput = {
  ...
  brmsId?: string | null;
  spaceSlug?: string | null;
  operationType: "ingest" | "query" | "lint" | "file-as-rule" | "index" | "distill" | "backfill";
  title?: string | null;
  prompt?: string | null;
...
type PaperclipProjectRuleDistillationInput = PaperclipSourceBundleInput & {
  autoApply?: boolean;
  expectedProjectRuleHash?: string | null;
  includeSupportingRules?: boolean;
};
...
type BRMSResourceKind = "agent" | "project";
...
type PaperclipEventIngestResult =
  ...
  | { status: "recorded"; sourceKind: BRMSEventIngestionSource; sourceId: string; cursorId: string; issueId: string };

type BRMSResourceBinding = {
...
function normalizeBRMSId(value: unknown): string {
  return stringField(value) ?? DEFAULT_BRMS_ID;
}
...
async function requirePaperclipIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; brmsId: string; spaceSlug?: string | null },
...
function stableSpaceId(input: { companyId: string; brmsId: string; slug: string }): string {
  const hex = createHash("md5")
    .update(`${input.companyId}:${input.brmsId}:${input.slug}`)
    .digest("hex");
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      `[Suppressed by Business Rules distillation security policy for this ${input.sourceKind}.]`,
      "",
...
async function resolvePaperclipDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<PaperclipSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: BRMSSpace },
): Promise<PaperclipDistillationLimits> {
...
async function assertSourceWithinConfiguredLimit(ctx: PluginContext, contents: string) {
  ...
  if (sourceBytes > maxSourceBytes) {
    throw new Error(`Source content is ${sourceBytes} bytes, which exceeds the configured Business Rules source limit of ${maxSourceBytes} bytes.`);
  }
}

function normalizeEventIngestionSettings(value: unknown): BRMSEventIngestionSettings {
  ...
  return {
    ...
    brmsId: normalizeBRMSId(record.brmsId),
    maxCharacters,
  };
}

function defaultPaperclipIngestionProfile(input: {
  space: Pick<BRMSSpace, "slug">;
  legacySettings?: BRMSEventIngestionSettings | null;
...
function normalizePaperclipIngestionProfile(
  value: unknown,
  input: { space: Pick<BRMSSpace, "slug">; legacySettings?: BRMSEventIngestionSettings | null },
): PaperclipIngestionProfileV1 {
...
async function profileForSpace(ctx: PluginContext, companyId: string, space: BRMSSpace): Promise<PaperclipIngestionProfileV1> {
...
function eventIngestionDedupKey(companyId: string, brmsId: string, spaceId: string, sourceKind: BRMSEventIngestionSource, sourceId: string) {
  return {
    scopeKind: "company" as const,
    scopeId: companyId,
    namespace: EVENT_INGESTION_DEDUP_NAMESPACE,
    stateKey: `${brmsId}:${spaceId}:${sourceKind}:${sourceId}`,
  };
}

export async function getEventIngestionSettings(ctx: PluginContext, companyId: string): Promise<BRMSEventIngestionSettings> {
...
export async function getPaperclipIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; brmsId?: string | null; spaceSlug?: string | null },
): Promise<PaperclipIngestionProfileRead> {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluatePaperclipProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalRuleCount = await countPaperclipHistoricalRules(ctx, {
    companyId: input.companyId,
    brmsId,
    spaceId: space.id,
  });
...
async function countPaperclipHistoricalRules(ctx: PluginContext, input: { companyId: string; brmsId: string; spaceId: string }): Promise<number> {
  const rows = await ctx.db.query<{ count: string | number }>(
    `SELECT count(*)::text AS count
       FROM ${ruleBindingTable(ctx)}
      WHERE company_id = $1 AND brms_id = $2 AND space_id = $3`,
    [input.companyId, input.brmsId, input.spaceId],
  );
...
async function countPaperclipProfileOverlaps(ctx: PluginContext, input: {
  ...
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, brmsId: input.brmsId });
  let overlaps = 0;
...
async function validatePaperclipIngestionProfile(ctx: PluginContext, input: {
  ...
  for (const scope of input.profile.sourceScopes) {
    if (scope.kind === "company_all" && input.space.slug !== DEFAULT_SPACE_SLUG) {
      throw new Error("Everything in the company is only available on the default brms space.");
    }
...
export async function updatePaperclipIngestionProfile(ctx: PluginContext, input: {
  ...
  await ctx.activity.log({
    ...
    entityType: "brms_space",
    entityId: space.id,
    metadata: {
      type: "plugin.brms.paperclip_ingestion_profile_updated",
      brmsId,
      spaceSlug: space.slug,
  ...
  return getPaperclipIngestionProfile(ctx, { companyId: input.companyId, brmsId, spaceSlug: space.slug });
}
...
export async function listPaperclipIngestionCandidates(ctx: PluginContext, input: PaperclipIngestionCandidatesInput): Promise<{
  ...
  const brmsId = normalizeBRMSId(input.brmsId);
  await requirePaperclipIngestionPolicy(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug }, "candidate_search");
...
export async function updateEventIngestionSettings(
  ctx: PluginContext,
  input: { companyId: string; settings: BRMSEventIngestionSettingsUpdate },
): Promise<BRMSEventIngestionSettings> {
  await requirePaperclipIngestionPolicy(ctx, {
    companyId: input.companyId,
    brmsId: normalizeBRMSId(input.settings.brmsId),
  ...
  const defaultSpace = await ensureDefaultSpace(ctx, { companyId: input.companyId, brmsId: next.brmsId });
...
function assertBRMSPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    throw new Error(`Invalid brms path: ${path}`);
  }
  if (
    trimmed !== ".gitignore" &&
    trimmed !== "BRMS.md" &&
    ...
    !trimmed.startsWith("brms/") &&
    !(options.allowMetadata && trimmed.startsWith(".paperclip/"))
  ) {
    throw new Error(`BRMS path must stay inside AGENTS.md, IDEA.md, raw/, or brms/: ${path}`);
  }
  return trimmed;
}

function assertRulePath(path: string): string {
  const normalized = assertBRMSPath(path);
  if (normalized !== "index.md" && normalized !== "log.md" && normalized !== "BRMS.md" && normalized !== "AGENTS.md" && normalized !== "IDEA.md" && !normalized.startsWith("brms/")) {
    throw new Error(`BRMS rule writes must target AGENTS.md, IDEA.md, or brms/: ${path}`);
  }
  if (!normalized.endsWith(".md") && !normalized.endsWith(".json")) {
    throw new Error(`BRMS rule path must end in .md or .json: ${path}`);
  }
  return normalized;
}

function assertRuleWriteAllowed(path: string, writer: WriteRuleInput["writer"] = "agent_tool"): void {
  if (writer !== "board_ui" && PROTECTED_BRMS_CONTROL_FILES.has(path)) {
    throw new Error(`Refusing to overwrite protected brms control file ${path}; board-managed edits must use the brms UI.`);
  }
}
...
function spaceTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "brms_spaces");
}

function bindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "brms_resource_bindings");
}
...
function ruleBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "paperclip_rule_bindings");
}
...
function brmsSpaceFromRow(row: BRMSSpaceRow): BRMSSpace {
  return {
    id: row.id,
...
function fallbackDefaultSpace(input: { companyId: string; brmsId: string }): BRMSSpace {
  return {
    id: stableSpaceId({ companyId: input.companyId, brmsId: input.brmsId, slug: DEFAULT_SPACE_SLUG }),
    companyId: input.companyId,
    brmsId: input.brmsId,
    ...
    rootFolderKey: BRMS_ROOT_FOLDER_KEY,
    pathPrefix: null,
...
export async function ensureDefaultSpace(ctx: PluginContext, input: { companyId: string; brmsId?: string | null }): Promise<BRMSSpace> {
  const brmsId = normalizeBRMSId(input.brmsId);
  const id = stableSpaceId({ companyId: input.companyId, brmsId, slug: DEFAULT_SPACE_SLUG });
  await ctx.db.execute(
    `INSERT INTO ${spaceTable(ctx)} AS brms_spaces
       (id, company_id, brms_id, slug, display_name, space_type, folder_mode, root_folder_key, path_prefix, access_scope, status, settings)
     VALUES ($1, $2, $3, 'default', 'default', 'local_folder', 'managed_subfolder', $4, NULL, 'shared', 'active', '{}'::jsonb)
     ON CONFLICT (company_id, brms_id, slug)
     DO UPDATE SET updated_at = brms_spaces.updated_at`,
    [id, input.companyId, brmsId, BRMS_ROOT_FOLDER_KEY],
  );
  const rows = await ctx.db.query<BRMSSpaceRow>(
    `SELECT id, company_id, brms_id, slug, display_name, space_type, folder_mode, root_folder_key,
       ...
       FROM ${spaceTable(ctx)}
      WHERE company_id = $1 AND brms_id = $2 AND slug = 'default'
      LIMIT 1`,
    [input.companyId, brmsId],
  );
  return rows[0] ? brmsSpaceFromRow(rows[0]) : fallbackDefaultSpace({ companyId: input.companyId, brmsId });
}

export async function resolveSpace(ctx: PluginContext, input: SpaceInput): Promise<BRMSSpace> {
  const brmsId = normalizeBRMSId(input.brmsId);
  const slug = normalizeSpaceSlug(input.spaceSlug);
  if (slug === DEFAULT_SPACE_SLUG) {
    return ensureDefaultSpace(ctx, { companyId: input.companyId, brmsId });
  }
  const rows = await ctx.db.query<BRMSSpaceRow>(
    `SELECT id, company_id, brms_id, slug, display_name, space_type, folder_mode, root_folder_key,
            path_prefix, configured_root_path, access_scope, owner_user_id, owner_agent_id, team_key,
            settings, status, created_at::text AS created_at, updated_at::text AS updated_at
       FROM ${spaceTable(ctx)}
      WHERE company_id = $1 AND brms_id = $2 AND slug = $3 AND status <> 'archived'
      LIMIT 1`,
    [input.companyId, brmsId, slug],
  );
  if (!rows[0]) throw new Error(`Business Rules space not found: ${slug}`);
  return brmsSpaceFromRow(rows[0]);
}

async function resolveSpaceAnyStatus(ctx: PluginContext, input: SpaceInput): Promise<BRMSSpace> {
  const brmsId = normalizeBRMSId(input.brmsId);
  const slug = normalizeSpaceSlug(input.spaceSlug);
  if (slug === DEFAULT_SPACE_SLUG) {
    return ensureDefaultSpace(ctx, { companyId: input.companyId, brmsId });
  ...
  const rows = await ctx.db.query<BRMSSpaceRow>(
    `SELECT id, company_id, brms_id, slug, display_name, space_type, folder_mode, root_folder_key,
            ...
            settings, status, created_at::text AS created_at, updated_at::text AS updated_at
       FROM ${spaceTable(ctx)}
      WHERE company_id = $1 AND brms_id = $2 AND slug = $3
      LIMIT 1`,
    [input.companyId, brmsId, slug],
  );
  if (!rows[0]) throw new Error(`Business Rules space not found: ${slug}`);
...
export async function listSpaces(ctx: PluginContext, input: { companyId: string; brmsId?: string | null }): Promise<{ spaces: BRMSSpace[] }> {
  const brmsId = normalizeBRMSId(input.brmsId);
  await ensureDefaultSpace(ctx, { companyId: input.companyId, brmsId });
  const rows = await ctx.db.query<BRMSSpaceRow>(
    `SELECT id, company_id, brms_id, slug, display_name, space_type, folder_mode, root_folder_key,
            path_prefix, configured_root_path, access_scope, owner_user_id, owner_agent_id, team_key,
            settings, status, created_at::text AS created_at, updated_at::text AS updated_at
       FROM ${spaceTable(ctx)}
      WHERE company_id = $1 AND brms_id = $2 AND status <> 'archived'
      ORDER BY CASE WHEN slug = 'default' THEN 0 ELSE 1 END, display_name, slug`,
    [input.companyId, brmsId],
  );
  const spaces = rows.length > 0 ? rows.map(brmsSpaceFromRow) : [fallbackDefaultSpace({ companyId: input.companyId, brmsId })];
  return { spaces };
}

export async function createSpace(ctx: PluginContext, input: CreateSpaceInput): Promise<{ status: "created"; space: BRMSSpace }> {
  const brmsId = normalizeBRMSId(input.brmsId);
  ...
  if (slug === DEFAULT_SPACE_SLUG) {
    return { status: "created", space: await ensureDefaultSpace(ctx, { companyId: input.companyId, brmsId }) };
  }
  ...
  await ctx.db.execute(
    `INSERT INTO ${spaceTable(ctx)}
       (id, company_id, brms_id, slug, display_name, space_type, folder_mode, root_folder_key, path_prefix, access_scope, settings, status)
  ...
  await upsertBRMSInstance(ctx, { companyId: input.companyId, brmsId });
  return { status: "created", space };
}

export async function updateSpace(ctx: PluginContext, input: UpdateSpaceInput): Promise<{ status: "ok"; space: BRMSSpace }> {
  const nextStatus = input.status ?? null;
  if (nextStatus !== null && nextStatus !== "active" && nextStatus !== "archived") {
    throw new Error("Business Rules space status must be active or archived.");
  }
  ...
  if (space.slug === DEFAULT_SPACE_SLUG && nextStatus === "archived") {
    throw new Error("The default Business Rules space cannot be archived.");
  }
  await ctx.db.execute(
  ...
  return { status: "ok", space: await resolveSpace(ctx, { companyId: input.companyId, brmsId: space.brmsId, spaceSlug: space.slug }) };
}

export async function archiveSpace(ctx: PluginContext, input: SpaceInput): Promise<{ status: "archived"; space: BRMSSpace }> {
  const space = await resolveSpace(ctx, input);
  if (space.slug === DEFAULT_SPACE_SLUG) throw new Error("The default Business Rules space cannot be archived.");
  await ctx.db.execute(
    `UPDATE ${spaceTable(ctx)}
        SET status = 'archived', updated_at = now()
      WHERE company_id = $1 AND brms_id = $2 AND slug = $3`,
    [input.companyId, space.brmsId, space.slug],
  );
  return { status: "archived", space: { ...space, status: "archived" } };
}

export function spaceRelativePath(space: Pick<BRMSSpace, "pathPrefix">, path: string): string {
  const normalized = path.replace(/^\/+/, "");
  return space.pathPrefix ? `${space.pathPrefix}/${normalized}` : normalized;
}

function logicalPathFromSpacePath(space: Pick<BRMSSpace, "pathPrefix">, path: string): string {
  if (!space.pathPrefix) return path;
  const prefix = `${space.pathPrefix}/`;
  return path.startsWith(prefix) ? path.slice(prefix.length) : path;
}

export async function spaceFolderStatus(ctx: PluginContext, input: SpaceInput): Promise<BRMSSpaceWithFolderStatus> {
  const space = await resolveSpace(ctx, input);
  const folder = await ctx.localFolders.status(input.companyId, BRMS_ROOT_FOLDER_KEY);
  return {
    ...space,
...
async function getResourceBinding(
  ctx: PluginContext,
  input: { companyId: string; brmsId: string; resourceKind: BRMSResourceKind; resourceKey: string },
): Promise<BRMSResourceBinding | null> {
  const rows = await ctx.db.query<{ resolved_id: string | null; metadata: unknown }>(
    `SELECT resolved_id, metadata
      ...
      WHERE company_id = $1
        ...
        AND resource_key = $4
      LIMIT 1`,
    [input.companyId, input.brmsId, input.resourceKind, input.resourceKey],
  );
  const row = rows[0];
...
async function upsertResourceBinding(
  ctx: PluginContext,
  input: { companyId: string; brmsId: string; resourceKind: BRMSResourceKind; resourceKey: string; resolvedId: string; metadata?: Record<string, unknown> },
) {
  await ctx.db.execute(
    `INSERT INTO ${bindingTable(ctx)} AS brms_resource_bindings
       (id, company_id, brms_id, resource_kind, resource_key, resolved_id, metadata)
     VALUES ($1, $2, $3, $4, $5, $6, $7::jsonb)
     ON CONFLICT (company_id, brms_id, resource_kind, resource_key)
...
function agentDetails(agent: Agent | null): BRMSAgentResource["details"] {
  return agent
    ? { name: agent.name, status: agent.status, adapterType: agent.adapterType ?? null, icon: agent.icon ?? null, urlKey: agent.urlKey ?? null }
    : null;
}

function projectDetails(project: Project | null): BRMSProjectResource["details"] {
  return project ? { name: project.name, status: project.status, color: project.color ?? null } : null;
}

function skillDetails(skill: PluginManagedSkillResolution["skill"]): BRMSSkillResource["details"] {
  return skill
...
function agentResource(input: {
  status: BRMSAgentResource["status"];
  source: BRMSAgentResource["source"];
  agent: Agent | null;
  defaultDrift?: BRMSAgentResource["defaultDrift"];
}): BRMSAgentResource {
  return {
    status: input.status,
    source: input.source,
    agentId: input.agent?.id ?? null,
    resourceKey: `${PLUGIN_ID}:agent:${BRMS_MAINTAINER_AGENT_KEY}`,
    agent: input.agent,
...
function projectResource(input: {
  status: BRMSProjectResource["status"];
  source: BRMSProjectResource["source"];
  project: Project | null;
}): BRMSProjectResource {
  return {
    status: input.status,
    ...
    resourceKey: `${PLUGIN_ID}:project:${BRMS_PROJECT_KEY}`,
    project: input.project,
    details: projectDetails(input.project),
  };
}

function skillResource(resolved: PluginManagedSkillResolution): BRMSSkillResource {
  return {
...
async function resolveSelectedAgent(ctx: PluginContext, companyId: string, binding: BRMSResourceBinding | null) {
...
async function resolveSelectedProject(ctx: PluginContext, companyId: string, binding: BRMSResourceBinding | null) {
  if (!binding?.resolvedId) return null;
...
function inferTitle(path: string, contents: string): string {
  ...
  const filename = path.split("/").pop()?.replace(/\.(md|json)$/i, "") ?? path;
  return filename.replace(/[-_]+/g, " ");
}

function inferRuleType(path: string): string | null {
  if (/^brms\/projects\/[^/]+\/standup\.md$/.test(path)) return "project-standup";
  const match = path.match(/^brms\/([^/]+)\//);
  return match?.[1] ?? (path === "index.md" || path === "brms/index.md" ? "index" : path === "log.md" || path === "brms/log.md" ? "log" : null);
}

function extractBRMSLinks(contents: string): string[] {
  const links = new Set<string>();
  const markdownLinkPattern = /\[[^\]]+\]\(([^)]+)\)/g;
  for (const match of contents.matchAll(markdownLinkPattern)) {
    const target = match[1]?.split("#")[0]?.trim();
    if (target && (target.startsWith("brms/") || target === "index.md" || target === "log.md" || target === "AGENTS.md" || target === "IDEA.md")) {
      links.add(target);
    }
  }
  const brmsTokenPattern = /\bbrms\/[A-Za-z0-9._/-]+\.md\b/g;
  for (const match of contents.matchAll(brmsTokenPattern)) {
    links.add(match[0]);
  }
  return [...links].sort();
}
...
async function readCurrentWithHash(
  ...
  companyId: string,
  path: string,
  space: Pick<BRMSSpace, "pathPrefix">,
): Promise<{ contents: string | null; hash: string | null }> {
  try {
    const contents = await ctx.localFolders.readText(companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, path));
...
async function filterReadableRows<T>(
  ...
  const checks: Array<T | null> = await Promise.all(rows.map(async (row): Promise<T | null> => {
    try {
      await ctx.localFolders.readText(companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, pathForRow(row)));
      return row;
    } catch {
      return null;
    }
...
async function listLocalFiles(ctx: PluginContext, input: { companyId: string; space: Pick<BRMSSpace, "pathPrefix">; relativePath: "brms" | "raw" }): Promise<PluginLocalFolderEntry[]> {
  try {
    const relativePath = spaceRelativePath(input.space, input.relativePath);
    const listing = await ctx.localFolders.list(input.companyId, BRMS_ROOT_FOLDER_KEY, {
      relativePath,
      recursive: true,
...
function mergeLocalRuleRows(rules: BRMSRuleRow[], entries: PluginLocalFolderEntry[]): BRMSRuleRow[] {
  const byPath = new Map(rules.map((rule) => [rule.path, rule]));
  for (const entry of entries) {
    const isSupported = entry.path.endsWith(".md") || entry.path.endsWith(".json");
    if (!isSupported || byPath.has(entry.path)) continue;
    byPath.set(entry.path, {
      path: entry.path,
      title: null,
      ruleType: inferRuleType(entry.path),
      backlinkCount: 0,
      sourceCount: 0,
      contentHash: null,
...
function mergeLocalSourceRows(sources: BRMSSourceRow[], entries: PluginLocalFolderEntry[]): BRMSSourceRow[] {
...
async function upsertBRMSInstance(ctx: PluginContext, input: { companyId: string; brmsId: string; rootPath?: string | null }) {
  await ctx.db.execute(
    `INSERT INTO ${tableName(ctx.db.namespace, "brms_instances")} AS brms_instances
       (id, company_id, brms_id, root_folder_key, configured_root_path, schema_version, settings, managed_agent_key, managed_project_key)
     VALUES ($1, $2, $3, $4, $5, 1, '{}'::jsonb, $6, $7)
     ON CONFLICT (company_id, brms_id)
     DO UPDATE SET configured_root_path = COALESCE(EXCLUDED.configured_root_path, brms_instances.configured_root_path),
...
async function upsertRuleMetadata(ctx: PluginContext, input: {
  companyId: string;
  brmsId: string;
  spaceId: string;
  path: string;
  ...
  const ruleType = inferRuleType(input.path);
  const backlinks = extractBRMSLinks(input.contents);
  const sourceRefs = Array.isArray(input.sourceRefs) ? input.sourceRefs : [];

  await ctx.db.execute(
    `INSERT INTO ${tableName(ctx.db.namespace, "brms_rules")}
       (id, company_id, brms_id, space_id, path, title, rule_type, frontmatter, source_refs, backlinks, content_hash, current_revision_id)
     VALUES ($1, $2, $3, $11, $4, $5, $6, '{}'::jsonb, $7::jsonb, $8::jsonb, $9, $10)
     ON CONFLICT (company_id, brms_id, space_id, path)
  ...
  await ctx.db.execute(
    `INSERT INTO ${tableName(ctx.db.namespace, "brms_rule_revisions")}
       (id, company_id, brms_id, space_id, rule_id, operation_id, path, content_hash, summary, metadata)
     VALUES ($1, $2, $3, $8, (SELECT id FROM ${tableName(ctx.db.namespace, "brms_rules")} WHERE company_id = $2 AND brms_id = $3 AND space_id = $8 AND path = $4), $7, $4, $5, $6, '{}'::jsonb)`,
    [revisionId, input.companyId, input.brmsId, input.path, hash, input.summary ?? null, input.operationId ?? null, input.spaceId],
  );

  return { title, ruleType, backlinks, hash, revisionId };
}

export async function writeBRMSRule(ctx: PluginContext, input: WriteRuleInput) {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const path = assertRulePath(input.path);
  assertRuleWriteAllowed(path, input.writer);
  ...
  await ctx.localFolders.writeTextAtomic(input.companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, path), input.contents);
  const metadata = await upsertRuleMetadata(ctx, {
    companyId: input.companyId,
    brmsId,
    spaceId: space.id,
    path,
  ...
  await upsertBRMSInstance(ctx, { companyId: input.companyId, brmsId });
  return { status: "ok", brmsId, spaceSlug: space.slug, path, previousHash: current.hash, ...metadata };
}

export async function captureBRMSSource(ctx: PluginContext, input: CaptureSourceInput) {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  ...
  await ctx.localFolders.writeTextAtomic(input.companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, rawPath), input.contents);
  await upsertBRMSInstance(ctx, { companyId: input.companyId, brmsId });
  const sourceId = randomUUID();
  await ctx.db.execute(
    `INSERT INTO ${tableName(ctx.db.namespace, "brms_sources")}
       (id, company_id, brms_id, space_id, source_type, title, url, raw_path, content_hash, status, metadata)
  ...
  return { status: "ok", sourceId, brmsId, spaceSlug: space.slug, rawPath, hash, title };
}

export async function getOverview(ctx: PluginContext, companyId: string): Promise<BRMSOverview> {
  const [defaultSpace, folder, managedAgent, managedProject, managedSkills] = await Promise.all([
    ensureDefaultSpace(ctx, { companyId, brmsId: DEFAULT_BRMS_ID }),
    ctx.localFolders.status(companyId, BRMS_ROOT_FOLDER_KEY),
    resolveBRMSAgentResource(ctx, companyId),
    resolveBRMSProjectResource(ctx, companyId),
    resolveBRMSSkillResources(ctx, companyId),
  ]);
  const operationRows = await ctx.db.query<{ count: string }>(
    `SELECT count(*)::text AS count FROM ${tableName(ctx.db.namespace, "brms_operations")} WHERE company_id = $1`,
    [companyId],
  );
...
export async function resolveBRMSAgentResource(
  ctx: PluginContext,
  companyId: string,
  options: { reconcileMissing?: boolean } = {},
): Promise<BRMSAgentResource> {
  const brmsId = DEFAULT_BRMS_ID;
  const binding = await getResourceBinding(ctx, {
    companyId,
    brmsId,
    resourceKind: "agent",
    resourceKey: BRMS_MAINTAINER_AGENT_KEY,
  });
  ...
  if (selectedAgent) {
    ...
    const managedResolution = source === "managed"
      ? await ctx.agents.managed.get(BRMS_MAINTAINER_AGENT_KEY, companyId)
      : null;
  ...
  const resolved = options.reconcileMissing
    ? await ctx.agents.managed.reconcile(BRMS_MAINTAINER_AGENT_KEY, companyId)
    : await ctx.agents.managed.get(BRMS_MAINTAINER_AGENT_KEY, companyId);
...
export async function resolveBRMSProjectResource(
  ctx: PluginContext,
  companyId: string,
  options: { reconcileMissing?: boolean } = {},
): Promise<BRMSProjectResource> {
  const brmsId = DEFAULT_BRMS_ID;
  const binding = await getResourceBinding(ctx, {
    companyId,
    brmsId,
  ...
  const resolved = options.reconcileMissing
    ? await ctx.projects.managed.reconcile(BRMS_PROJECT_KEY, companyId)
    : await ctx.projects.managed.get(BRMS_PROJECT_KEY, companyId);
...
export async function resolveBRMSSkillResources(
  ctx: PluginContext,
  companyId: string,
  options: { reconcileMissing?: boolean } = {},
): Promise<BRMSSkillResource[]> {
  return Promise.all(
    BRMS_MANAGED_SKILL_KEYS.map(async (skillKey) => {
      const resolved = options.reconcileMissing
...
export async function reconcileBRMSAgentResource(ctx: PluginContext, companyId: string): Promise<BRMSAgentResource> {
  const resolved = await ctx.agents.managed.reconcile(BRMS_MAINTAINER_AGENT_KEY, companyId);
  if (resolved.agentId) {
    await upsertResourceBinding(ctx, {
      companyId,
      brmsId: DEFAULT_BRMS_ID,
...
export async function resetBRMSAgentResource(ctx: PluginContext, companyId: string): Promise<BRMSAgentResource> {
  const resolved = await ctx.agents.managed.reset(BRMS_MAINTAINER_AGENT_KEY, companyId);
  if (resolved.agentId) {
    await upsertResourceBinding(ctx, {
      companyId,
      brmsId: DEFAULT_BRMS_ID,
...
export async function selectBRMSAgentResource(ctx: PluginContext, input: { companyId: string; agentId: string }): Promise<BRMSAgentResource> {
  const agent = await ctx.agents.get(input.agentId, input.companyId);
  if (!agent || agent.status === "terminated") {
    throw new Error("Selected BRMS Maintainer agent was not found or is terminated.");
  }
  await upsertResourceBinding(ctx, {
    companyId: input.companyId,
    brmsId: DEFAULT_BRMS_ID,
...
export async function reconcileBRMSProjectResource(ctx: PluginContext, companyId: string): Promise<BRMSProjectResource> {
  const resolved = await ctx.projects.managed.reconcile(BRMS_PROJECT_KEY, companyId);
...
export async function resetBRMSProjectResource(ctx: PluginContext, companyId: string): Promise<BRMSProjectResource> {
  const resolved = await ctx.projects.managed.reset(BRMS_PROJECT_KEY, companyId);
  if (resolved.projectId) {
    await upsertResourceBinding(ctx, {
      companyId,
      brmsId: DEFAULT_BRMS_ID,
...
export async function reconcileBRMSSkillResources(ctx: PluginContext, companyId: string): Promise<BRMSSkillResource[]> {
  return resolveBRMSSkillResources(ctx, companyId, { reconcileMissing: true });
}

export async function resetBRMSSkillResources(ctx: PluginContext, companyId: string): Promise<BRMSSkillResource[]> {
  return Promise.all(
    BRMS_MANAGED_SKILL_KEYS.map(async (skillKey) => {
...
export async function reconcileBRMSRoutineResources(
  ctx: PluginContext,
  companyId: string,
): Promise<{
  managedAgent: BRMSAgentResource;
  ...
  const [managedAgent, managedProject] = await Promise.all([
    reconcileBRMSAgentResource(ctx, companyId),
    reconcileBRMSProjectResource(ctx, companyId),
  ]);
...
export async function selectBRMSProjectResource(ctx: PluginContext, input: { companyId: string; projectId: string }): Promise<BRMSProjectResource> {
  const project = await ctx.projects.get(input.projectId, input.companyId);
  if (!project) {
    throw new Error("Selected Business Rules project was not found.");
  }
  await upsertResourceBinding(ctx, {
    companyId: input.companyId,
    brmsId: DEFAULT_BRMS_ID,
...
export async function listBRMSAgentOptions(ctx: PluginContext, companyId: string): Promise<BRMSResourceOption[]> {
...
export async function listBRMSProjectOptions(ctx: PluginContext, companyId: string): Promise<BRMSResourceOption[]> {
...
export async function bootstrapBRMSRoot(ctx: PluginContext, input: BootstrapInput) {
  const brmsId = DEFAULT_BRMS_ID;
  const defaultSpace = await ensureDefaultSpace(ctx, { companyId: input.companyId, brmsId });
  const configureFolder = (path: string) => ctx.localFolders.configure({
    companyId: input.companyId,
    folderKey: BRMS_ROOT_FOLDER_KEY,
    path,
    access: "readWrite",
    requiredDirectories: [...REQUIRED_BRMS_DIRECTORIES],
    requiredFiles: [...REQUIRED_BRMS_FILES],
  });
  const currentFolder = input.path
    ? null
    : await ctx.localFolders.status(input.companyId, BRMS_ROOT_FOLDER_KEY);
  const folder = input.path
    ...
    : currentFolder?.configured && currentFolder.path
      ...
      : currentFolder ?? await ctx.localFolders.status(input.companyId, BRMS_ROOT_FOLDER_KEY);

  const writtenFiles: string[] = [];
  const preservedFiles: string[] = [];
  for (const file of BOOTSTRAP_FILES) {
    const path = assertBRMSPath(file.path, { allowMetadata: true });
    try {
      await ctx.localFolders.readText(input.companyId, BRMS_ROOT_FOLDER_KEY, path);
      preservedFiles.push(path);
      continue;
    } catch {
      // Missing files are initialized below. Existing files are intentionally preserved.
    }
    await ctx.localFolders.writeTextAtomic(input.companyId, BRMS_ROOT_FOLDER_KEY, file.path, file.contents);
    writtenFiles.push(path);
  }

  await upsertBRMSInstance(ctx, { companyId: input.companyId, brmsId, rootPath: folder.path });
  const managedSkills = await reconcileBRMSSkillResources(ctx, input.companyId);
  const [managedAgent, managedProject] = await Promise.all([
    reconcileBRMSAgentResource(ctx, input.companyId),
    reconcileBRMSProjectResource(ctx, input.companyId),
  ]);
  await ctx.state.set(
    {
      scopeKind: "company",
...
async function bootstrapSpaceFiles(ctx: PluginContext, companyId: string, space: BRMSSpace) {
  const writtenFiles: string[] = [];
  const preservedFiles: string[] = [];
  for (const file of BOOTSTRAP_FILES) {
    const path = assertBRMSPath(file.path, { allowMetadata: true });
    ...
    try {
      await ctx.localFolders.readText(companyId, BRMS_ROOT_FOLDER_KEY, physicalPath);
      preservedFiles.push(path);
      continue;
    } catch {
      // Missing files are initialized below. Existing files are intentionally preserved.
    }
    await ctx.localFolders.writeTextAtomic(companyId, BRMS_ROOT_FOLDER_KEY, physicalPath, file.contents);
    writtenFiles.push(path);
  }
  return { writtenFiles, preservedFiles };
}

function operationSpaceRoot(space: BRMSSpace): string {
  return space.pathPrefix ? `${space.rootFolderKey}/${space.pathPrefix}` : `${space.rootFolderKey} root`;
}

function operationBillingContext(brmsId: string, space: BRMSSpace): string {
  return space.slug === DEFAULT_SPACE_SLUG
    ? `plugin-brms:${brmsId} (space ${space.slug})`
    : `plugin-brms:${brmsId}:${space.slug}`;
}

function operationBillingCode(brmsId: string, space: BRMSSpace): string {
  return space.slug === DEFAULT_SPACE_SLUG ? `plugin-brms:${brmsId}` : `plugin-brms:${brmsId}:${space.slug}`;
}

function operationIssueOriginId(input: { brmsId: string; space: BRMSSpace; operationId: string }): string {
  return input.space.slug === DEFAULT_SPACE_SLUG
    ? `brms:${input.brmsId}:operation:${input.operationId}`
    : `brms:${input.brmsId}:space:${input.space.slug}:operation:${input.operationId}`;
}

function operationTitleWithSpace(title: string, space: BRMSSpace): string {
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  ...
  return [
    ...
    `BRMS ID: ${input.brmsId}`,
    `Space: ${input.space.displayName} (${input.space.slug})`,
    `Space root: ${operationSpaceRoot(input.space)}`,
    `Billing context: ${operationBillingContext(input.brmsId, input.space)}`,
    "",
    "Space isolation requirement:",
    `- Pass brmsId \`${input.brmsId}\` and spaceSlug \`${input.space.slug}\` on every Business Rules tool call.`,
    "- Treat all paths in the prompt as relative to this space root.",
    paperclipDerived
      ? "- Paperclip-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Paperclip-derived rules into a non-default space."
      : "- Manual ingest, query, lint, index, and file-as-rule operations follow the named destination space. Do not cross into another space unless the operation explicitly asks for a multi-space sweep.",
    "",
    input.prompt ?? "Created by the Business Rules plugin.",
  ].join("\n");
}
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requirePaperclipIngestionPolicy(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
    : await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const managedAgent = await resolveBRMSAgentResource(ctx, input.companyId, { reconcileMissing: true });
  const managedProject = await resolveBRMSProjectResource(ctx, input.companyId, { reconcileMissing: true });
  const operationId = randomUUID();
  const title = operationTitleWithSpace(input.title ?? `Business Rules ${input.operationType}`, space);
  const originId = operationIssueOriginId({ brmsId, space, operationId });
  const operationContext = { brmsId, space, operationType: input.operationType, operationId, prompt: input.prompt };
  ...
  const issue = await ctx.issues.create({
    ...
    billingCode: operationBillingCode(brmsId, space),
  ...
  await ctx.db.execute(
    `INSERT INTO ${tableName(ctx.db.namespace, "brms_operations")}
       (id, company_id, brms_id, space_id, operation_type, status, hidden_issue_id, project_id, run_ids, cost_cents, warnings, metadata)
  ...
  return { operationId, brmsId, spaceSlug: space.slug, issue };
}

function isLlmBRMSOperationIssue(issue: Issue): boolean {
...
async function upsertPaperclipDistillationCursor(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS paperclip_distillation_cursors
       (id, company_id, brms_id, space_id, source_scope, scope_key, project_id, root_issue_id, source_kind, last_observed_at, pending_event_count, metadata)
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'paperclip_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, brms_id, space_id, source_scope, scope_key, source_kind)
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      LIMIT 1`,
    [input.companyId, input.brmsId, input.spaceId, scope.sourceScope, scope.scopeKey],
  );
...
function isActiveDistillationProject(project: Project): boolean {
  ...
  if (project.managedByPlugin?.resourceKey === BRMS_PROJECT_KEY) return false;
  return true;
}
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await requirePaperclipIngestionPolicy(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
...
async function listPaperclipBundleIssues(ctx: PluginContext, input: PaperclipSourceBundleInput): Promise<Issue[]> {
  const filterAndSort = (issues: Issue[]) =>
    issues
      .filter((issue) => !isLlmBRMSOperationIssue(issue))
...
export async function assemblePaperclipSourceBundle(ctx: PluginContext, input: PaperclipSourceBundleInput): Promise<PaperclipSourceBundle> {
  ...
  const space = await requirePaperclipIngestionPolicy(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createPaperclipDistillationRun(ctx: PluginContext, input: PaperclipDistillationRunInput) {
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationRunTable(ctx)}
       (id, company_id, brms_id, space_id, cursor_id, work_item_id, project_id, root_issue_id, source_window_start, source_window_end, source_hash, status, operation_issue_id, retry_count, cost_cents, warnings, metadata)
  ...
  await ctx.db.execute(
    `INSERT INTO ${sourceSnapshotTable(ctx)}
       (id, company_id, brms_id, space_id, distillation_run_id, project_id, root_issue_id, source_hash, max_characters, clipped, source_refs, bundle_markdown, metadata)
  ...
  return { status: "source_ready" as const, brmsId, spaceSlug: space.slug, cursorId, runId, snapshotId, bundle, estimatedCostCents };
}
    ...
    await ctx.db.execute(
      `UPDATE ${distillationCursorTable(ctx)}
        ...
        WHERE company_id = $1
          ...
          AND space_id = $7
          AND id = $6`,
      [input.companyId, brmsId, input.runId, input.sourceWindowEnd, input.sourceHash, input.cursorId, space.id],
    );
  }
...
export async function createPaperclipDistillationWorkItem(ctx: PluginContext, input: PaperclipDistillationWorkItemInput) {
  const brmsId = normalizeBRMSId(input.brmsId);
  assertPaperclipSourceScopePayload(input);
  const space = await requirePaperclipIngestionPolicy(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true });
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS paperclip_distillation_work_items
       (id, company_id, brms_id, space_id, work_item_kind, status, priority, project_id, root_issue_id, requested_by_issue_id, idempotency_key, metadata)
     VALUES ($1, $2, $3, $11, $4, 'pending', $5, $6, $7, $8, $9, $10::jsonb)
     ON CONFLICT (company_id, brms_id, space_id, idempotency_key)
  ...
  return { status: "pending", workItemId: itemId, brmsId, spaceSlug: space.slug, kind: input.kind, sourceScope: scope.sourceScope };
}
...
function projectRuleSlug(input: { project: Project | null; rootIssue: Issue | null }): string {
...
function standupRuleContents(input: {
  project: Project | null;
  rootIssue: Issue | null;
  issues: Issue[];
  bundle: PaperclipSourceBundle;
  rulePath: string;
  durableRulePath: string;
}): string {
  ...
  return [
    ...
    `project: ${JSON.stringify(projectRuleSlug(input))}`,
    ...
    "## Links",
    "",
    `- Durable project overview: [[${input.durableRulePath}]]`,
    ...input.bundle.sourceRefs.slice(0, 12).map(sourceRefMarkdown),
    "",
  ].filter((line): line is string => line !== null).join("\n");
}

function projectRuleContents(input: {
  project: Project | null;
  rootIssue: Issue | null;
  issues: Issue[];
  bundle: PaperclipSourceBundle;
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This rule synthesizes Paperclip issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    "## References",
    "",
    `- Current standup: [[${input.rulePath.replace(/\/index\.md$/, "/standup.md")}]]`,
    ...input.bundle.sourceRefs.slice(0, 12).map(sourceRefMarkdown),
    "",
  ].filter((line): line is string => line !== null).join("\n");
}

function decisionsRuleContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: PaperclipSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Paperclip Project";
...
function historyRuleContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: PaperclipSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Paperclip Project";
...
function updateProjectIndexContents(current: string | null, input: { rulePath: string; standupPath: string; title: string }): string {
  const base = current?.trimEnd() || "# Index\n\n## Sources\n\n_(none yet)_\n\n## Projects\n\n_(none yet)_\n\n## Entities\n\n_(none yet)_\n\n## Concepts\n\n_(none yet)_\n\n## Synthesis\n\n_(none yet)_";
  const entry = `- [[${input.rulePath}]] — ${input.title} project overview. Current executive standup: [[${input.standupPath}]].`;
  ...
  const existingLines = projectsMatch[2]
    ...
    .filter((line) => line.trim() && line.trim() !== "_(none yet)_" && !line.includes(input.rulePath) && !line.includes(input.standupPath));
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; rulePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  const base = current?.trimEnd() || "# Log\n\nAppend-only chronological record of brms operations.";
...
async function readRuleBinding(ctx: PluginContext, input: { companyId: string; brmsId: string; spaceId: string; rulePath: string }) {
  const rows = await ctx.db.query<{ last_applied_source_hash: string | null }>(
    `SELECT last_applied_source_hash
      ...
      WHERE company_id = $1
        ...
        AND rule_path = $4
      LIMIT 1`,
    [input.companyId, input.brmsId, input.spaceId, input.rulePath],
  );
  return rows[0] ?? null;
}

async function upsertRuleBinding(ctx: PluginContext, input: {
  companyId: string;
  brmsId: string;
  spaceId: string;
  spaceSlug: string;
  ...
  await ctx.db.execute(
    `INSERT INTO ${ruleBindingTable(ctx)} AS paperclip_rule_bindings
       (id, company_id, brms_id, space_id, project_id, root_issue_id, rule_path, last_applied_source_hash, last_distillation_run_id, metadata)
     VALUES ($1, $2, $3, $10, $4, $5, $6, $7, $8, $9::jsonb)
     ON CONFLICT (company_id, brms_id, space_id, rule_path)
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = paperclip_rule_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export async function distillPaperclipProjectRule(ctx: PluginContext, input: PaperclipProjectRuleDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const slug = projectRuleSlug({ project, rootIssue });
  const projectDir = `brms/projects/${slug}`;
  const standupPath = assertRulePath(`${projectDir}/standup.md`);
  const rulePath = assertRulePath(`${projectDir}/index.md`);
  const run = await createPaperclipDistillationRun(ctx, input);
  const bundle = run.bundle;
  const current = await readCurrentWithHash(ctx, input.companyId, rulePath, space);
  assertExpectedHash(input.expectedProjectRuleHash, current.hash, rulePath);
  ...
  const existingBinding = await readRuleBinding(ctx, { companyId: input.companyId, brmsId, spaceId: space.id, rulePath });
  ...
  const standupContents = standupRuleContents({ project, rootIssue, issues, bundle, rulePath: standupPath, durableRulePath: rulePath });
  const projectContents = projectRuleContents({ project, rootIssue, issues, bundle, rulePath });
  const indexCurrent = await readCurrentWithHash(ctx, input.companyId, "brms/index.md", space);
  const logCurrent = await readCurrentWithHash(ctx, input.companyId, "brms/log.md", space);
  ...
  const patches: PaperclipDistillationPatch[] = [
    patchForRule({ path: standupPath, operationType: "standup_update", currentHash: standupCurrent.hash, contents: standupContents, bundle, confidence, warnings, humanReviewRequired: reviewRequired }),
    patchForRule({ path: rulePath, operationType: "project_rule_distill", currentHash: current.hash, contents: projectContents, bundle, confidence, warnings, humanReviewRequired: reviewRequired }),
    patchForRule({ path: "brms/index.md", operationType: "index_refresh", currentHash: indexCurrent.hash, contents: indexContents, bundle, confidence: "high", warnings: [], humanReviewRequired: false }),
    patchForRule({ path: "brms/log.md", operationType: "log_append", currentHash: logCurrent.hash, contents: logContents, bundle, confidence: "high", warnings: [], humanReviewRequired: false }),
  ];

  if (input.includeSupportingRules !== false) {
    const hasDecisions = issues.some((issue) => hasDecisionSignal(`${issue.title}\n${issueDescription(issue)}`));
    if (hasDecisions) {
      const decisionsPath = assertRulePath(`${projectDir}/decisions.md`);
      const decisionsCurrent = await readCurrentWithHash(ctx, input.companyId, decisionsPath, space);
      patches.push(patchForRule({
        path: decisionsPath,
        operationType: "decision_distill",
        currentHash: decisionsCurrent.hash,
        contents: decisionsRuleContents({ project, rootIssue, issues, bundle }),
        bundle,
        confidence,
        warnings,
    ...
    const historyPath = assertRulePath(`${projectDir}/history.md`);
    ...
    patches.push(patchForRule({
      ...
      contents: historyRuleContents({ project, rootIssue, issues, bundle }),
      bundle,
      confidence,
      warnings,
      humanReviewRequired: reviewRequired,
  ...
  for (const patch of patches) {
    ...
    appliedRules.push(patch.rulePath);
  }
  await recordPaperclipDistillationOutcome(ctx, {
    companyId: input.companyId,
    brmsId,
...
function truncateEventSource(contents: string, maxCharacters: number): string {
  ...
  return `${contents.slice(0, maxCharacters)}\n\n[Truncated by Business Rules event ingestion policy at ${maxCharacters} characters.]\n`;
}
...
function rawPathForPaperclipEvent(input: {
  sourceKind: BRMSEventIngestionSource;
  issue: Issue;
  label: string;
  contents: string;
  event: PluginEvent;
...
async function recordPaperclipCursorObservation(ctx: PluginContext, input: {
  ...
  await ctx.state.set(eventIngestionDedupKey(input.companyId, input.brmsId, input.space.id, input.sourceKind, input.sourceId), {
...
async function routePaperclipCursorObservation(ctx: PluginContext, input: {
  ...
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, brmsId: DEFAULT_BRMS_ID });
  ...
  for (const space of spaces) {
    ...
    if (await ctx.state.get(eventIngestionDedupKey(input.companyId, space.brmsId, space.id, input.sourceKind, input.sourceId))) {
      continue;
    }
...
export async function handlePaperclipEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<PaperclipEventIngestResult> {
  ...
  if (isLlmBRMSOperationIssue(issue)) return { status: "skipped", reason: "plugin_operation" };
...
function queryStreamChannel(operationId: string): string {
  return `brms:query:${operationId}`;
}

function buildQueryPrompt(input: { companyId: string; brmsId: string; space: BRMSSpace; question: string }): string {
  return [
    QUERY_PROMPT,
    `Company ID: ${input.companyId}`,
    `BRMS ID: ${input.brmsId}`,
    `Space: ${input.space.displayName} (${input.space.slug})`,
    `Space root: ${operationSpaceRoot(input.space)}`,
    `Tool arguments: always pass brmsId \`${input.brmsId}\` and spaceSlug \`${input.space.slug}\`.`,
    "Use the Business Rules plugin tools against that space only. Read brms/index.md first with brms_read_rule, then use brms_search, brms_read_rule, brms_list_sources, and brms_read_source as needed.",
    "Cite the brms rule paths and raw source paths you used. If the brms does not contain enough evidence, say that directly.",
...
async function markOperation(ctx: PluginContext, input: {
  ...
  runId?: string | null;
  warning?: string | null;
  affectedRules?: unknown[] | null;
  metadata?: Record<string, unknown> | null;
}) {
  await ctx.db.execute(
    `UPDATE ${tableName(ctx.db.namespace, "brms_operations")}
        SET status = $3,
            run_ids = CASE WHEN $4::jsonb = '[]'::jsonb THEN run_ids ELSE run_ids || $4::jsonb END,
            warnings = CASE WHEN $5::jsonb = '[]'::jsonb THEN warnings ELSE warnings || $5::jsonb END,
            affected_rules = CASE WHEN $6::jsonb = '[]'::jsonb THEN affected_rules ELSE $6::jsonb END,
    ...
    [
      ...
      jsonArrayParam(input.affectedRules ?? []),
      jsonParam(input.metadata ?? {}),
    ],
  );
}
...
export async function startBRMSQuerySession(ctx: PluginContext, input: QuerySessionInput) {
  ...
  const operation = await createOperationIssue(ctx, {
    ...
    spaceSlug: space.slug,
    operationType: "query",
    title: input.title ?? `Query Business Rules: ${question.slice(0, 72)}`,
    prompt: question,
  });
  ...
  if (!agentId) {
    const warning = "No configured BRMS Maintainer agent is available for this company.";
  ...
  if (!agent || agent.status === "paused" || agent.status === "terminated" || agent.status === "pending_approval") {
    const warning = agent
      ? `BRMS Maintainer agent is not invokable while status is ${agent.status}.`
      : "BRMS Maintainer agent could not be loaded.";
    await markOperation(ctx, {
      companyId: input.companyId,
      operationId: operation.operationId,
      status: "blocked",
  ...
  const session = await ctx.agents.sessions.create(agentId, input.companyId, {
    taskKey: `plugin:${PLUGIN_ID}:session:brms:${brmsId}:query:${operation.operationId}`,
    reason: "Business Rules query session",
  });
  await ctx.db.execute(
    `INSERT INTO ${tableName(ctx.db.namespace, "brms_query_sessions")}
       (id, company_id, brms_id, space_id, hidden_issue_id, agent_session_id, status, filed_outputs)
     VALUES ($1, $2, $3, $6, $4, $5, 'active', '[]'::jsonb)`,
    [operation.operationId, input.companyId, brmsId, operation.issue.id, session.sessionId, space.id],
  );

  const prompt = buildQueryPrompt({ companyId: input.companyId, brmsId, space, question });
  ...
  const sendResult = await ctx.agents.sessions.sendMessage(session.sessionId, input.companyId, {
    ...
    onEvent: (event) => {
      ...
      if (isTerminalSessionEvent(event)) {
        ...
        void ctx.db.execute(
          `UPDATE ${tableName(ctx.db.namespace, "brms_query_sessions")}
              SET status = $3,
                  updated_at = now()
            WHERE company_id = $1 AND id = $2`,
...
export async function fileQueryAnswerAsRule(ctx: PluginContext, input: FileQueryAnswerInput) {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const path = assertRulePath(input.path);
  ...
  const operation = await createOperationIssue(ctx, {
    ...
    title: `File Business Rules answer as ${path}`,
    prompt: input.question ?? answer ?? `Write ${path}`,
  });
  const result = await writeBRMSRule(ctx, {
    companyId: input.companyId,
    brmsId,
    spaceSlug: space.slug,
    path,
    ...
    await ctx.db.execute(
      `UPDATE ${tableName(ctx.db.namespace, "brms_query_sessions")}
          SET filed_outputs = filed_outputs || $3::jsonb,
              updated_at = now()
        WHERE company_id = $1 AND id = $2`,
      [input.companyId, input.querySessionId, jsonArrayParam([affectedRule])],
    );
  }
...
export async function registerBRMSTools(ctx: PluginContext) {
  ctx.tools.register("brms_search", {
    displayName: "Search BRMS",
    description: "Search indexed brms rule and source metadata.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_search")?.parametersSchema ?? { type: "object" },
    ...
    const brmsId = normalizeBRMSId(input.brmsId);
    const space = await resolveSpace(ctx, { companyId, brmsId, spaceSlug: input.spaceSlug as string | null | undefined });
    ...
    const rows = await ctx.db.query<{ kind: string; path: string; title: string | null; match_text: string | null }>(
      `SELECT 'rule' AS kind, path, title, rule_type AS match_text
         FROM ${tableName(ctx.db.namespace, "brms_rules")}
        WHERE company_id = $1 AND brms_id = $2 AND space_id = $5 AND (lower(path) LIKE lower($3) OR lower(coalesce(title, '')) LIKE lower($3))
       UNION ALL
       SELECT 'source' AS kind, raw_path AS path, title, source_type AS match_text
         FROM ${tableName(ctx.db.namespace, "brms_sources")}
        WHERE company_id = $1 AND brms_id = $2 AND space_id = $5 AND (lower(raw_path) LIKE lower($3) OR lower(coalesce(title, '')) LIKE lower($3) OR lower(coalesce(url, '')) LIKE lower($3))
       ORDER BY kind, path
       LIMIT $4`,
      [companyId, brmsId, `%${query}%`, limit, space.id],
    );
    return {
      content: rows.length ? rows.map((row) => `${row.kind}: ${row.path}${row.title ? ` - ${row.title}` : ""}`).join("\n") : "No brms matches found.",
      data: { companyId, brmsId, spaceSlug: space.slug, query, results: rows },
    };
  });

  ctx.tools.register("brms_read_rule", {
    displayName: "Read BRMS Rule",
    description: "Read a markdown brms rule from the configured local brms root.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_read_rule")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
    const space = await resolveSpace(ctx, { companyId, brmsId, spaceSlug: input.spaceSlug as string | null | undefined });
    const path = assertRulePath(requireString(input.path, "path"));
    const contents = await ctx.localFolders.readText(companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, path));
    return { content: contents, data: { companyId, brmsId, spaceSlug: space.slug, path, hash: contentHash(contents) } };
  });

  ctx.tools.register("brms_write_rule", {
    displayName: "Write BRMS Rule",
    description: "Atomically write a markdown brms rule after plugin path validation.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_write_rule")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const result = await writeBRMSRule(ctx, {
      companyId: requireString(input.companyId, "companyId"),
      brmsId: stringField(input.brmsId),
  ...
  ctx.tools.register("brms_propose_patch", {
    displayName: "Propose BRMS Patch",
    description: "Return a structured proposed rule write without changing files.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_propose_patch")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
  ...
  ctx.tools.register("brms_list_sources", {
    displayName: "List BRMS Sources",
    description: "Return captured raw source metadata from the plugin index.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_list_sources")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
    ...
    const rows = await ctx.db.query<{ raw_path: string; title: string | null; source_type: string; url: string | null; content_hash: string }>(
      `SELECT raw_path, title, source_type, url, content_hash
         FROM ${tableName(ctx.db.namespace, "brms_sources")}
        WHERE company_id = $1 AND brms_id = $2 AND space_id = $4
        ORDER BY created_at DESC
        LIMIT $3`,
      [companyId, brmsId, limit, space.id],
    );
    return {
      content: rows.length ? rows.map((row) => `${row.raw_path}${row.title ? ` - ${row.title}` : ""}`).join("\n") : "No sources captured yet.",
      data: { companyId, brmsId, spaceSlug: space.slug, sources: rows },
    };
  });

  ctx.tools.register("brms_read_source", {
    displayName: "Read BRMS Source",
    description: "Read a captured raw source from the configured local brms root.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_read_source")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
    ...
    const contents = await ctx.localFolders.readText(companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, rawPath));
    return { content: contents, data: { companyId, brmsId, spaceSlug: space.slug, rawPath, hash: contentHash(contents) } };
  });

  ctx.tools.register("brms_append_log", {
    displayName: "Append BRMS Log",
    description: "Append a maintenance note to brms/log.md.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_append_log")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
    ...
    let current = "";
    try {
      current = await ctx.localFolders.readText(companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, "brms/log.md"));
    } catch {
      current = "# Log\n\nAppend-only chronological record of brms operations.\n";
    }
    const next = `${current.trimEnd()}\n\n- ${new Date().toISOString()} ${entry}\n`;
    await ctx.localFolders.writeTextAtomic(companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, "brms/log.md"), next);
    await upsertRuleMetadata(ctx, {
      companyId,
      brmsId,
      spaceId: space.id,
      path: "brms/log.md",
    ...
    return { content: "Appended log entry", data: { companyId, brmsId, spaceSlug: space.slug, hash: contentHash(next) } };
  });

  ctx.tools.register("brms_update_index", {
    displayName: "Update BRMS Index",
    description: "Atomically replace brms/index.md with optional hash conflict checks.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_update_index")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const result = await writeBRMSRule(ctx, {
      companyId: requireString(input.companyId, "companyId"),
    ...
    return { content: "Updated brms/index.md", data: result };
  });

  ctx.tools.register("brms_list_backlinks", {
    displayName: "List BRMS Backlinks",
    description: "Return indexed backlinks for a brms rule.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_list_backlinks")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
    ...
    const rows = await ctx.db.query<{ path: string; title: string | null }>(
      `SELECT path, title
         FROM ${tableName(ctx.db.namespace, "brms_rules")}
        WHERE company_id = $1 AND brms_id = $2 AND space_id = $4 AND backlinks ? $3
        ORDER BY path
        LIMIT 200`,
      [companyId, brmsId, path, space.id],
    );
    return {
      content: rows.length ? rows.map((row) => `${row.path}${row.title ? ` - ${row.title}` : ""}`).join("\n") : "No backlinks indexed.",
      data: { companyId, brmsId, spaceSlug: space.slug, path, backlinks: rows },
    };
  });

  ctx.tools.register("brms_list_rules", {
    displayName: "List BRMS Rules",
    description: "Return the known rule index from plugin metadata.",
    parametersSchema: ctx.manifest.tools?.find((tool) => tool.name === "brms_list_rules")?.parametersSchema ?? { type: "object" },
  }, async (params: unknown): Promise<ToolResult> => {
    const input = params as ToolParams;
    const companyId = requireString(input.companyId, "companyId");
    const brmsId = normalizeBRMSId(input.brmsId);
    const space = await resolveSpace(ctx, { companyId, brmsId, spaceSlug: input.spaceSlug as string | null | undefined });
    const rows = await ctx.db.query<{ path: string; title: string | null; rule_type: string | null }>(
      `SELECT path, title, rule_type FROM ${tableName(ctx.db.namespace, "brms_rules")} WHERE company_id = $1 AND brms_id = $2 AND space_id = $3 ORDER BY path LIMIT 200`,
      [companyId, brmsId, space.id],
    );
    return {
      content: rows.length ? rows.map((row) => `${row.path}${row.title ? ` - ${row.title}` : ""}`).join("\n") : "No rules indexed yet.",
      data: { companyId, brmsId, spaceSlug: space.slug, rules: rows },
    };
  });
}
...
type BRMSTemplateFile = (typeof TEMPLATE_FILES)[number];

function isTemplateFile(value: string): value is BRMSTemplateFile {
  return (TEMPLATE_FILES as readonly string[]).includes(value);
}

export type BRMSRuleRow = {
  path: string;
  title: string | null;
...
export async function listRules(ctx: PluginContext, input: {
  companyId: string;
  brmsId?: string | null;
  spaceSlug?: string | null;
  ruleType?: string | null;
  includeRaw?: boolean;
  limit?: number | null;
}): Promise<{ rules: BRMSRuleRow[]; sources: BRMSSourceRow[] }> {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const limit = normalizeLimit(input.limit, 500, LOCAL_BROWSE_FILE_LIMIT);
  const params: unknown[] = [input.companyId, brmsId, space.id];
  let ruleFilter = "";
  if (input.ruleType) {
    params.push(input.ruleType);
    ruleFilter = ` AND rule_type = $${params.length}`;
  }
  params.push(limit);
  const limitIndex = params.length;
  const ruleRows = await ctx.db.query<{
    path: string;
    title: string | null;
    ...
    `SELECT path, title, rule_type, backlinks, source_refs, content_hash, updated_at::text AS updated_at
       FROM ${tableName(ctx.db.namespace, "brms_rules")}
      WHERE company_id = $1 AND brms_id = $2 AND space_id = $3${ruleFilter}
      ORDER BY path
      LIMIT $${limitIndex}`,
    params,
  );
  const readableRuleRows = await filterReadableRows(ctx, input.companyId, space, ruleRows, (row) => row.path);
  const rules: BRMSRuleRow[] = readableRuleRows.map((row) => ({
    path: row.path,
    title: row.title,
  ...
  let rulesWithLocalFiles = rules;
  if (!input.ruleType) {
    const brmsFiles = await listLocalFiles(ctx, { companyId: input.companyId, space, relativePath: "brms" });
    rulesWithLocalFiles = mergeLocalRuleRows(rules, brmsFiles);
  }

  let sources: BRMSSourceRow[] = [];
  if (input.includeRaw) {
    sources = (await listSources(ctx, { companyId: input.companyId, brmsId, spaceSlug: space.slug, limit, onlyReadable: true })).sources;
    sources = mergeLocalSourceRows(sources, await listLocalFiles(ctx, { companyId: input.companyId, space, relativePath: "raw" }));
  }
  return { rules: rulesWithLocalFiles, sources };
}
...
export async function listSources(ctx: PluginContext, input: {
  ...
  const rows = await ctx.db.query<{ raw_path: string; title: string | null; source_type: string; url: string | null; status: string; created_at: string }>(
    `SELECT raw_path, title, source_type, url, status, created_at::text AS created_at
      ...
      ORDER BY created_at DESC
      LIMIT $3`,
    [input.companyId, brmsId, limit, space.id],
  );
...
export async function readBRMSRule(ctx: PluginContext, input: { companyId: string; brmsId?: string | null; spaceSlug?: string | null; path: string }) {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const path = assertBRMSPath(input.path);
  const contents = await ctx.localFolders.readText(input.companyId, BRMS_ROOT_FOLDER_KEY, spaceRelativePath(space, path));
  const meta = await ctx.db.query<{ title: string | null; rule_type: string | null; backlinks: unknown; source_refs: unknown; updated_at: string }>(
    `SELECT title, rule_type, backlinks, source_refs, updated_at::text AS updated_at
       FROM ${tableName(ctx.db.namespace, "brms_rules")}
      WHERE company_id = $1 AND brms_id = $2 AND space_id = $4 AND path = $3
      LIMIT 1`,
    [input.companyId, brmsId, path, space.id],
  );
  ...
  return {
    ...
    ruleType: row?.rule_type ?? inferRuleType(path),
...
export async function readTemplate(ctx: PluginContext, input: { companyId: string; path: string }) {
  ...
  try {
    const contents = await ctx.localFolders.readText(input.companyId, BRMS_ROOT_FOLDER_KEY, input.path);
...
export async function writeTemplate(ctx: PluginContext, input: { companyId: string; path: string; contents: string }) {
  ...
  await ctx.localFolders.writeTextAtomic(input.companyId, BRMS_ROOT_FOLDER_KEY, input.path, input.contents);
...
export type DistillationRuleBindingRow = {
  id: string;
  rulePath: string;
  projectId: string | null;
  projectName: string | null;
...
export type DistillationOverview = {
  ...
  ruleBindings: DistillationRuleBindingRow[];
...
function affectedRulePathsFromRunMetadata(metadata: Record<string, unknown>, fallbackBindings: DistillationRuleBindingRow[], runId: string): string[] {
  const explicit = jsonArray(metadata.affectedRules ?? metadata.rulePaths ?? metadata.affected_rules)
    .map((entry) => {
...
export async function getDistillationOverview(ctx: PluginContext, input: {
  ...
  const cursorRows = await ctx.db.query<{
    ...
    `SELECT cursor.id,
      ...
      WHERE cursor.company_id = $1 AND cursor.brms_id = $2 AND cursor.space_id = $3
      ORDER BY cursor.updated_at DESC
      LIMIT 200`,
    [input.companyId, brmsId, space.id],
  );
  ...
  const runRows = await ctx.db.query<{
    ...
    `SELECT run.id,
      ...
      WHERE run.company_id = $1 AND run.brms_id = $2 AND run.space_id = $4
      ORDER BY run.created_at DESC
      LIMIT $3`,
    [input.companyId, brmsId, runLimit, space.id],
  );
  ...
  const workItemRows = await ctx.db.query<{
    ...
    `SELECT id, work_item_kind, status, priority, project_id, root_issue_id, metadata,
      ...
      WHERE company_id = $1 AND brms_id = $2 AND space_id = $3 AND status IN ('pending', 'review_required', 'in_progress', 'failed')
      ORDER BY created_at DESC
      LIMIT 100`,
    [input.companyId, brmsId, space.id],
  );
  ...
  const bindingRows = await ctx.db.query<{
    ...
    `SELECT binding.id,
       ...
       FROM ${ruleBindingTable(ctx)} binding
      ...
      WHERE binding.company_id = $1 AND binding.brms_id = $2 AND binding.space_id = $3
  ...
  const ruleBindings: DistillationRuleBindingRow[] = bindingRows.map((row) => ({
    id: row.id,
  ...
  const runs: DistillationRunRow[] = runRows.map((row) => {
    ...
    return {
      ...
      affectedRulePaths: affectedRulePathsFromRunMetadata(metadata, ruleBindings, row.id),
      createdAt: row.created_at,
      updatedAt: row.updated_at,
    };
  });
...
export async function getDistillationRuleProvenance(ctx: PluginContext, input: {
  companyId: string;
  brmsId?: string | null;
  spaceSlug?: string | null;
  rulePath: string;
}): Promise<{
  binding: DistillationRuleBindingRow | null;
  runs: DistillationRunRow[];
  ...
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  const overview = await getDistillationOverview(ctx, { companyId: input.companyId, brmsId, spaceSlug: space.slug });
  const binding = overview.ruleBindings.find((row) => row.rulePath === input.rulePath) ?? null;
  if (!binding) {
  ...
  const relatedRuns = overview.runs.filter((run) => {
    ...
    return run.affectedRulePaths.includes(binding.rulePath);
  });
  ...
  if (binding.lastDistillationRunId) {
    const snapshotRows = await ctx.db.query<{
      ...
      `SELECT id, distillation_run_id, source_hash, max_characters, clipped, source_refs, metadata, created_at::text AS created_at
        ...
        WHERE company_id = $1 AND brms_id = $2 AND space_id = $4 AND distillation_run_id = $3
        ORDER BY created_at DESC
        LIMIT 1`,
      [input.companyId, brmsId, binding.lastDistillationRunId, space.id],
    );
    if (snapshotRows[0]) {
...
export async function listOperations(ctx: PluginContext, input: {
  ...
  status?: string | null;
  limit?: number | null;
}): Promise<{ operations: BRMSOperationRow[] }> {
  const brmsId = normalizeBRMSId(input.brmsId);
  const space = await resolveSpace(ctx, { companyId: input.companyId, brmsId, spaceSlug: input.spaceSlug });
  ...
  const rows = await ctx.db.query<{
    ...
    `SELECT op.id, op.operation_type, op.status, op.hidden_issue_id, op.project_id,
            op.run_ids, op.cost_cents, op.warnings, op.affected_rules, op.metadata,
       ...
       FROM ${tableName(ctx.db.namespace, "brms_operations")} op
       LEFT JOIN public.issues issue ON issue.id = op.hidden_issue_id
      WHERE op.company_id = $1 AND op.brms_id = $2 AND op.space_id = $3${filterSql}
  ...
  return {
    operations: rows.map((row) => ({
      ...
      affectedRules: Array.isArray(row.affected_rules) ? row.affected_rules : [],
      metadata: jsonObject(row.metadata),
      createdAt: row.created_at,
      updatedAt: row.updated_at,
    })),
